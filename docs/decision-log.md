...
- Impact: 현재 상태는 `scenario-ready / not-measured`이며 baseline 또는 개선 성과 수치는 없다. 이 세션에서는 seed, k6, Docker, DB를 실행하지 않고 production Java/API/권한/응답/오류/트랜잭션/Entity/DB/Flyway/의존성을 변경하지 않는다. 실제 측정과 production N+1 수정은 별도 PM 승인 게이트 뒤에만 진행한다.
