...
### 2026-07-14 - Issue #196 Prayer/Poll Before Scenario Boundary

- Context: Issue #196의 production N+1 제거 전에 실제 API와 1,000명 fixture를 기준으로 재현 가능한 before 계약이 필요하며, 사용자는 이번 개발 세션 범위를 시나리오 준비까지만 승인했다.
- Decision: stable `datasetId`와 lowercase 실행별 immutable `fixtureRunId`를 분리하고, primary campus ACTIVE 1,000명과 isolation campus ACTIVE 50명을 새 row로만 생성한다. Prayer는 40조×25명과 제출 800/미제출 200, Poll은 5 options와 응답 800/미응답 200, comments 200, templates 40×8을 고정한다. 기존 row와 다른 fixture run의 row는 수정·삭제하지 않는다. 다섯 visibility fixture는 exact ID/campus와 run ID에서 파생한 title을 모두 확인한 하나의 SQL statement로만 보정하고, 일부 불일치와 재-shaping은 거부한다.
- Decision: Prayer current season/groups/assignable/admin weekly board/member weekly board, Poll member list/detail/results/comments/isolation, Poll admin common list/detail/results/comments/missing-members/templates/isolation을 endpoint별 독립 k6 프로세스로 측정한다. 상위 runner는 명시적으로 전달된 `prayer`, `poll-member`, `poll-admin`, 또는 `all`만 허용하고 `all`에서는 `prayer -> poll-member -> poll-admin`을 순차 실행한다. app/DB의 동일 실제 Compose project label을 검증한 뒤 seed/shaper/runner가 모두 canonical `/tmp/faithlog-performance-{composeProject}.lock`을 사용하고 다른 k6 부재를 확인하므로 같은 shared Compose에서 다른 성능 이슈와 병렬 실행하지 않는다.
- Decision: 각 endpoint는 runtime 필수 `WARMUP_VUS/WARMUP_DURATION` 별도 process를 먼저 끝내고 성공 시에만 `MEASURED_VUS/MEASURED_DURATION` process를 시작한다. measured evidence window 전 token을 재발급하고 JWT `exp`가 phase와 safety margin을 덮는지 raw token 저장·출력 없이 확인한다. 측정마다 명시한 새 immutable `executionRunId` 디렉터리를 사용하고 report sink도 exclusive create로 기존 evidence를 삭제·재사용·덮어쓰기하지 않는다. caller의 stale token env를 제거하고 credential은 shell-only로 내린 뒤 login/DB child에만 inline 전달하며 k6에는 새 token만 준다.
- Decision: 각 endpoint는 exact custom Trend `p50 <= p95 <= p99 <= max`, Counter positive request count/throughput, Rate failure exact 0, app/PostgreSQL CPU/RAM, Hibernate SQL query count/queriesPerRequest/repeated normalized SQL, PostgreSQL exact table set의 estimated row/scan/fetch/write counter, 실제 app image tag/immutable image ID와 published target port 및 Compose project/service/config-hash label을 기록한다. k6 direct v2와 `values` metric shape를 모두 허용하되 latency는 finite/nonnegative, throughput/count는 positive여야 한다. DB snapshot은 timestamp 증가, planner/analyze 상태 불변, 각 counter finite/nonnegative/monotonic 및 각 table의 insert/update/delete delta 개별 0을 요구한다. 현재 observer PID, attested app-container client address, measured k6 PID 및 Docker proxy만 제외하고 같은 command/application name의 다른 client를 포함한 measured-window 외부 DB/HTTP activity와 autoanalyze/planner 변화는 report를 non-adoptable로 만든다. 목록 개수·정렬·editable/myGroup·campus isolation 및 Poll visibility/status/time-window/results correctness failure는 0건이어야 한다. warmup 실패는 해당 warmup artifact만 보존하고 measured phase를 시작하지 않는다. measured k6/resource/activity sampler/time-window/log/after-DB-snapshot 실패 또는 필수 evidence 누락·오형식은 reason과 함께 `accepted=false`/`measurementStatus=rejected` report로 보존하고 runner를 비정상 종료한다.
- Decision: DB snapshot은 exact 27-table field schema, 8개 non-empty planner key, database/address/port/postmaster identity, analyze/autoanalyze/vacuum/autovacuum count와 null-or-valid timestamp를 모두 필수로 하며 maintenance와 runtime identity drift를 거부한다. PostgreSQL 누적 counter는 SQL decimal string과 BigInt exact delta로 처리한다. app/DB/Redis container ID·image ID·StartedAt과 PostgreSQL/Redis process identity는 warmup 전, measured 직전·직후, 최종 report 전에 동일해야 한다. Sampling cadence/max-gap과 exclusive-window 채택 방식은 사용자 미승인이므로 default를 두지 않고, 별도 승인 전에는 clean evidence도 `automaticAdoption=false`, `conditional-not-adoptable`로 fail-closed한다. BASE_URL, app/DB/Redis container, expected service/image identities, source/Flyway/Redis identity도 기본값 없는 runtime 승인 입력이며 actual/manifest identity와 exact 결속한다.
- Decision: target runtime 입력 계약은 runner뿐 아니라 DB를 쓰는 seed/shape와 direct k6 entrypoint에도 동일하게 적용한다. 하나라도 누락되면 API/Docker/DB/request 전에 실패한다. Summarizer는 1초/2초 상수를 사용하지 않고 runtime에서 승인된 positive sampling interval/max-gap(`maxGap >= interval`)과 evidence coverage를 검증한다. Resource evidence는 metadata의 app/DB/Redis exact 3-container name/full-ID set과 finite/nonnegative CPU, strict RAM bytes, `0..100` percent만 허용한다. 이 검증을 통과해도 automatic adoption은 사용자 별도 승인 전까지 비활성이다.
- Decision: seed/shape/run은 app/DB immutable container ID·image ID·StartedAt, Compose project/service/config hash, published endpoint, PostgreSQL database/address/port/postmaster identity를 lock 전에 승인 snapshot으로 잡고 canonical project lock 획득 직후 exact 동일성을 재검증한 뒤에만 login/API fixture write/Poll shaping/k6를 허용한다. `BASE_URL`은 승인된 해석 규칙이 없는 `localhost` 대신 explicit numeric loopback `127.0.0.1` 또는 `[::1]`만 허용하며, 공통 validator가 requested address family의 exact/wildcard Docker binding과 host port가 exact 1개인지 검증한다. RAM evidence는 strict unit의 safe nonnegative used/limit bytes, positive limit, `used <= limit`, reported percent `0..100`을 요구하고 parsed bytes에서 canonical percent와 decimal byte report를 만든다.
- Decision: 2026-07-16 최신 `origin/develop` `6796ed146244d8f3f5b5dd7048ebe16865084a97` 기준으로 scenario dataset을 v2로 갱신한다. generic Poll list는 pagination 없는 배열과 ID 내림차순을 그대로 검증하고, 별도 MEAL management list는 explicit `page=0&size=100&sort=id,desc`, 90일 archive 기본 제외/명시 포함을 검증한다. #200의 CUSTOM admin, COFFEE active-duty creator, MEAL active-duty `manageableByMe` matrix와 `createdBy` 비노출을 normal member/admin/두 COFFEE duty/MEAL duty별로 고정한다. #202 기준 27개 public application table 전부 RLS enabled, FORCE RLS/policy 0, JDBC database-owner bypass 무영향과 Flyway 11을 source identity로 검증한다. #206의 stable-ordering 원칙 중 이 목록에 관련된 부분은 generic Poll ID 내림차순과 MEAL explicit `id,desc`로만 반영하며 billing production 계약은 변경하지 않는다.
- Decision: current-develop Compose의 필수 Redis 의존성을 포함해 seed/shape/run은 app/DB/Redis의 full container ID·image ID·StartedAt·Compose project/service/config hash, PostgreSQL identity/Flyway/RLS/JDBC owner/pgss state, Redis process `run_id`와 port를 lock 전후 및 runner measured 전후·final에 exact 대조한다. pgss는 available 또는 unavailable 상태가 window 전체에서 일관되면 evidence로 기록하되 malformed/coherence 위반/drift는 거부한다. k6 v2 direct/values Counter·Rate·Trend, decimal-string/BigInt DB counters, exact 3-container resource cadence/window, immutable fixture/execution namespace와 최초 `primaryRejectionReason` 보존을 하나의 issue-local fake/static 계약으로 감사한다.
- Decision: #192~#199 test/scenario 보정은 병렬 수행할 수 있지만 shared Docker stack을 사용하는 실제 seed/load는 PM exclusive window에서 이슈별 순차 실행한다. #196은 이번 단계에서 실제 Docker/DB/HTTP/k6를 실행하지 않고 `automaticAdoption=false`, `conditional-not-adoptable`, `scenario-ready / not-measured`를 유지한다.
- Decision: #196 actual handoff에서는 `/private/tmp/FaithLog-perf-206-deploy`의 clean detached `6796ed146244d8f3f5b5dd7048ebe16865084a97` checkout과 그 뒤 생성된 same-runtime app image를 #192와 같은 operational provenance로 사용할 수 있다. OCI revision label/repo digest/`git.properties`가 없어 image-alone cryptographic source proof가 없다는 limitation은 기록하되 새 blocker로 취급하지 않는다. Runner의 runtime-required source revision, immutable app/DB/Redis image/container ID, StartedAt, Compose label과 DB/Redis process continuity gate는 유지한다.
- Decision: clean endpoint evidence는 채택 승인이 아니라 `conditional-not-adoptable`이므로 explicit mode의 다음 endpoint로 순차 진행하고, requested scope 전체 수집 뒤 runner가 exit 2로 끝난다. `rejected`, malformed report, warmup/runtime/k6/correctness/DB/resource failure는 최초 endpoint에서 fail-closed 중단한다. 이는 automatic adoption을 활성화하지 않으며 실제 load는 여전히 PM exclusive slot과 사용자 승인 workload가 필요하다.
- Decision: runner는 k6/resource sampler/integrity/runtime 등 operational status가 모두 0인 뒤에만 conditional continuation을 기록한다. Optional `PERF_REPORT_ROOT`는 local artifact base만 바꾸며 runner가 그 아래 immutable `{fixtureRunId}/{executionRunId}`를 붙인다. workload, target, service/image identity, credential에는 어떤 fallback도 추가하지 않는다.
- Decision: 2026-07-17 `g02` partial attempt에서 endpoint 2개 report가 약 1.6 GiB, `prayer_groups` SQL artifact 하나가 약 1.3 GiB로 증가해 27 endpoint 수집이 구조적으로 불가능함을 확인했다. 기존 plain SQL artifact와 post-phase 전체 `docker logs` 회수 방식은 폐기한다. 다음 actual부터 measured 전 live follower를 시작해 first/final sentinel 사이 statement-only SQL을 create-only gzip으로 직접 저장하고, compressed SHA-256과 uncompressed byte/line/statement count를 decimal string으로 기록한다. Validator/summarizer는 gzip을 line-by-line stream으로만 읽으며 bind/extract, sentinel, child, gzip/digest/count 오류를 secret-free fail-closed 처리한다.
- Decision: gzip-after-the-fact만으로는 Docker daemon 내부 log 팽창을 막지 못하므로 runtime prep은 app에만 Docker `local` log driver의 runtime-approved `max-size`/`max-file`/compression을 적용하고 그 exact config와 maximum retained bytes를 prep/fixture/runner identity에 결속한다. 매 endpoint 전후에는 `remainingEndpoints * (compressedSqlCap + nonSqlCap) + daemonLogBound + safetyHeadroom`을 exact decimal/BigInt로 계산하며 safety headroom은 최소 2 GiB다. 어떤 cap/projection도 default가 없고 실패하면 다음 endpoint를 시작하지 않는다. 기존 30/180 maintenance quiet, credentials-file, resource sampler, runtime identity, `automaticAdoption=false` 계약은 완화하지 않는다.
- Decision: live follower는 final sentinel을 관찰한 뒤 자신이 Docker logs child를 중단한 경우에만 `SIGTERM` 또는 Docker Desktop CLI의 exact exit `143`을 정상 경계로 인정한다. final 전 natural/nonzero 종료, final 뒤 별도 failure, timeout, missing/truncated gzip은 계속 거부한다. Installed Docker smoke는 sentinel 2개와 비민감 SQL statement 1줄만 app log에 기록해 이 경계를 확인하며 HTTP/DB/seed/shape/k6/lifecycle은 실행하지 않는다.
- Decision: PM은 보존된 `g02`의 observed compressed SQL 최대 12,842,803 bytes와 non-SQL 최대 509,406 bytes를 근거로 다음 fresh actual의 runtime cap을 SQL gzip 32 MiB/endpoint, non-SQL 8 MiB/endpoint로 명시 승인했다. App Docker `local` log는 `max-size=64m`, `max-file=3`, compression enabled로 제한하고 safety headroom은 2 GiB를 유지한다. 27 endpoint exact projection은 3,481,272,320 bytes이며 cap 초과와 projection 부족은 기존처럼 fail-closed한다. 이 cap 승인은 partial `g02`를 baseline으로 채택하거나 `automaticAdoption=false`를 변경하지 않는다.
- Decision: Fresh `h02` actual은 runtime prep/fixture/shape와 첫 endpoint를 통과했으나 두 번째 `prayer_groups`의 resource boundary validator가 measured 종료 후 SQL 후처리 동안 계속 수집된 마지막 sample을 경계로 사용해 false rejection했다. 종료 직후 nearest sample은 413 ms였고 unchanged max gap 3초 안이었으므로, validator는 latest sample at-or-before start와 earliest sample at-or-after end를 선택한다. 선택 경계의 max gap, 선택 구간 minimum count, 전체 sample identity/strict monotonic/consecutive max gap은 모두 유지한다. `h02` read-only 재검증은 policy rejection만 남겼지만 원 실행은 영구 rejected/non-reusable이고, 다음 actual은 fresh `h03`만 사용한다.
- Decision: `h03` prep ID는 잘못된 tooling HEAD preflight에서 lifecycle/fixture/load 0건으로 소비돼 재사용하지 않는다. Fresh `h04`는 prep/fixture/shape와 첫 두 measured Prayer endpoint를 HTTP/custom failure 0으로 완료했으나 `prayer_assignable` warmup에서 정상 primary/isolation 공통 creator를 isolation 오염으로 오판했다. Fixture가 두 campus를 같은 ADMIN으로 생성해 membership user ID 교집합이 정확히 1명이므로, Prayer assignable/weekly-board 격리 deny-set은 `isolation user IDs - primary user IDs`로 계산한다. 격리 전용 49명 차단, exact primary order/count, malformed/duplicate ID fail-closed는 유지한다. `h04`는 rejected/non-reusable이며 다음 actual은 fresh `h05`만 사용한다.
- Decision: SQL capture shell/Node/Docker follower와 sentinel Docker child는 parent runner 환경을 상속하지 않는다. `env -i` 뒤 `PATH`/`HOME`/`TMPDIR`, optional `DOCKER_CONFIG`/`DOCKER_HOST`, required non-secret sentinel/capture 값만 전달하며 credential, token, DB password와 caller unknown env는 실행형 fake orchestration에서 0건이어야 한다.
- Decision: resource sampler는 create-only ready marker를 기록한 뒤에만 measured k6를 시작한다. Runner는 sampler child 또는 ready timeout을 bounded wait로 먼저 판정하며, k6가 짧거나 host scheduling이 지연돼 stop marker가 sampler startup보다 먼저 생기는 endpoint 간 race를 허용하지 않는다.
- Decision: resource stream sampler는 exact 3-role args와 fresh stop/ready path를 검증하고 stdin reader를 초기화한 뒤에만 create-only 0600 ready marker를 기록한다. Runner는 sampler child exit와 ready marker를 bounded wait하고 ready 전에는 measured k6를 시작하지 않는다. Startup delay/failure/timeout은 first-stop이며 cadence/max-gap과 final complete tick 계약을 완화하지 않는다.
- Impact: 최초 시나리오 준비 단계의 Docker/DB/load 0건 기록은 당시 상태다. 이후 PM exclusive actual의 `g01`/`g02`/`h02` 등 모든 evidence와 ID는 삭제·수정·재사용하지 않으며 partial attempt는 rejected evidence일 뿐 valid baseline이 아니다. 현재도 `scenario-ready / not-measured`이고 production Java/API/권한/응답/오류/트랜잭션/Entity/DB/Flyway/의존성 변경과 개선 성과 수치는 없다.
