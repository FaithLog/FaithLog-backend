...
- Decision: DB snapshot은 exact 27-table field schema, 8개 non-empty planner key, database/address/port/postmaster identity, analyze/autoanalyze/vacuum/autovacuum count와 null-or-valid timestamp를 모두 필수로 하며 maintenance와 runtime identity drift를 거부한다. PostgreSQL 누적 counter는 SQL decimal string과 BigInt exact delta로 처리한다. app/DB container ID·image ID·StartedAt과 PostgreSQL identity는 warmup 전, measured 직전·직후, 최종 report 전에 동일해야 한다. Sampling cadence/max-gap과 exclusive-window 채택 방식은 사용자 미승인이므로 default를 두지 않고, 별도 승인 전에는 clean evidence도 `automaticAdoption=false`, `conditional-not-adoptable`로 fail-closed한다. BASE_URL, app/DB container, expected service labels와 exact app image도 기본값 없는 runtime 승인 입력이며 actual/manifest identity와 exact 결속한다.
- Decision: target runtime 입력 계약은 runner뿐 아니라 DB를 쓰는 seed/shape와 direct k6 entrypoint에도 동일하게 적용한다. 하나라도 누락되면 API/Docker/DB/request 전에 실패한다. Summarizer는 1초/2초 상수를 사용하지 않고 runtime에서 승인된 positive sampling interval/max-gap(`maxGap >= interval`)과 evidence coverage를 검증한다. Resource evidence는 metadata의 app/DB container exact 2개와 finite/nonnegative CPU·memory percent만 허용한다. 이 검증을 통과해도 automatic adoption은 사용자 별도 승인 전까지 비활성이다.
