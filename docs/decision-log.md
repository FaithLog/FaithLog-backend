...
- Decision: PM 독립 리뷰 finding에 따라 runner는 inspect로 확인한 actual Compose project의 canonical `/tmp/faithlog-performance-{project}.lock`만 원자 획득하고 device/inode 소유권이 유지된 빈 directory만 비재귀 해제한다. `WARM_RUNS`는 기본값 없는 필수 runtime 입력이다. inspect `POSTGRES_DB`와 psql server address/port/database/postmaster 시작 시각은 실행 전후 container identity와 fail-closed로 일치해야 한다. 시작 snapshot이 불완전하거나 외부 activity가 있으면 EXPLAIN을 시작하지 않고, planner setting, postmaster, analyze/autoanalyze, `n_mod_since_analyze`, external activity가 전후 오염되면 report를 `invalid/pending`으로 기록하고 non-zero로 종료해 채택을 차단한다.
- Decision: inventory SQL은 `exact-current-production`, `reconstructed-current-query`, `synthetic-candidate`로 분류한다. production before 개선 근거는 captured Hibernate SQL fingerprint와 1:1 연결된 exact SQL만 가능하며, 현재 24개 inventory는 reconstructed 2개와 synthetic 22개로 exact/production-before eligible이 0개다. plan hash는 runtime metric을 제외하되 재귀 parent-child topology를 포함한다.
- Decision: PM 2차 독립 리뷰에 따라 cross-issue artifact 7개는 각 이슈의 기존 measured/adoptable 상태, dataset/fixture/member/anchor identity, contained regular-file path와 SHA-256을 모두 통과해야 한다. runner는 anchor 관계·exact cardinality, Flyway/column/constraint/index schema fingerprint, analyze/vacuum/autovacuum/visibility, immutable SQL/control/production-source hash와 Git clean state를 read-only preflight/continuity gate로 검증한다. 전체 측정 구간 observer는 psql 생성 전 CSPRNG application/token/label을 준비하고 exact PID/application/backend-start를 bind하며 외부 activity 또는 observer 종료를 즉시 중단한다. start/runtime 거절은 credential/raw error 없이 0600 `invalid-pending-*` report로 남기고, child를 reap하지 못하면 canonical lock을 유지한다.
- Decision: PM 3차 독립 리뷰에 따라 현재 #193 `conditional-shared-stack`, #196/#199 `conditional-not-adoptable` artifact는 #194에서 pending으로 fail closed하며, 실제 도구가 만들지 않는 성공 status나 일반 `approved` 필드를 채택 근거로 만들지 않는다. 실제 측정·PM/사용자 채택 뒤 정확한 per-issue schema와 별도 승인 증거를 연결하는 계약은 별도 승인 작업으로 남긴다. observer interval은 기본값 없는 `ACTIVITY_SAMPLE_INTERVAL_MS` 양의 정수 runtime 입력이며 값은 실행 창별 사용자 승인이 필요하다. observer와 boundary snapshot은 PostgreSQL instance 전체 client backend를 보고 다른 database activity를 별도 reason으로 거부한다. pre-lock inspect identity는 canonical lock 직후와 전체 EXPLAIN 뒤에 container ID/name, image, startedAt, Compose 및 network identity까지 exact 비교하며 교체 시 fail closed한다.
- Decision: 최신 전체-diff 독립 리뷰 finding에 따라 타 role visibility 제한으로 `pg_stat_activity.state=NULL`인 client backend도 idle이 아닌 오염으로 fail closed한다. 시작 DB identity psql은 lock 직후 immutable inspect와 직후 재-inspect 사이에 두어 교체 시 schema/anchor/EXPLAIN 0회로 중단하고, 종료 immutable inspect는 planner/schema/DB after-state 수집 뒤 수행한다. 승인 sampling interval에는 새 임의 상한을 두지 않고 SIGTERM이 pending delay를 즉시 깨워 final sample/report를 보존한다.
- Decision: observer SIGTERM이 진행 중 최대 2초 sample 뒤 final sample을 한 번 더 수행할 수 있으므로 observer에만 두 sample timeout과 1초 여유를 합친 5초 graceful reap bound를 적용한다. 측정 cadence 범위나 measured EXPLAIN psql의 기존 종료 정책은 바꾸지 않는다.
- Decision: 2026-07-16 최신 `origin/develop` `6796ed146244d8f3f5b5dd7048ebe16865084a97` 기준으로 #194 scenario identity를 재결속한다. #200 COFFEE creator/duty/account/configuration, #201 archive cutoff, #202 V11 RLS와 runtime JDBC role, #206 stable ordering을 24개 diagnostic SQL·anchor·source/schema manifest에 연결한다. 승인된 Compose project/service/image ID/reference, PostgreSQL target/credential, warm workload와 observer cadence는 모두 기본값 없는 runtime 입력이다.
- Decision: #192/#193/#195/#196/#197/#198/#199 현재 artifact는 모두 별도 사용자·PM approval evidence schema가 없어 pending으로 거부한다. path/regular-file/JSON/schema/dataset/fixture/member/anchor/hash 검증은 전체 artifact에 먼저 수행하지만, 임의 status나 `approved=true`로 채택을 우회하지 않는다. PostgreSQL 누적 bigint counter는 decimal-string으로 수집해 BigInt로 비교하고 `pg_stat_statements`의 available/unavailable 상태, Flyway V11·index·RLS/JDBC security fingerprint, pre/post-lock/final Compose·DB role continuity가 바뀌면 automatic adoption 없이 machine-readable `invalid-pending-*` report로 거부한다.
- Decision: k6 v2 metric 수학, HTTP exact count/latency, Redis continuity, Docker CPU/RAM resource sampling은 PostgreSQL diagnostic EXPLAIN runner가 생성하는 evidence가 아니므로 #194에는 적용하지 않는다. 관련 tool·fallback을 새로 추가하지 않으며 실제 load는 PM의 순차 exclusive window에서 별도 이슈 계약으로만 수행한다.
- Decision: 자체 전체-diff 감사에서 `liveTuples`와 `deadTuples`를 decimal-string으로 수집만 하고 전후 채택 gate에서 비교하지 않는 gap을 확인했다. `nModSinceAnalyze`, vacuum/autovacuum count, live/dead tuple, all-visible page의 모든 누적 counter를 BigInt로 비교하고 하나라도 변하면 중복 없는 machine-readable reason으로 invalid 처리한다.
- Decision: 2026-07-17 사용자의 성능 테스트·backend 최적화 계속 진행 승인과 이력서용 lean 측정 결정에 따라, current-develop fixture에서 핵심 post-change SQL만 `EXPLAIN (ANALYZE, BUFFERS)`로 비교한다. 후보 index는 transaction 안에서 생성하고 즉시 rollback하며 shared DB에는 남기지 않는다.
- Decision: 실제 plan으로 효과가 확인된 `charge_items(campus_id,payment_category,source_type,source_id)`, `charge_items(campus_id,payment_category,status,user_id)`, `campus_members(user_id,id)`만 V12에 추가한다. #196/#197/#198/#199의 기존 index로 충분한 경로, leading-wildcard, account-scoped wide index, partial/include index는 추가하지 않는다.
- Impact: V12는 세 secondary index만 추가하고 API/Entity/권한/transaction/dependency를 변경하지 않는다. 임시 PostgreSQL DB에서 V1-V12 clean Flyway와 exact index definition을 검증하고 임시 DB를 삭제했다. API-level 성과는 통합 runtime의 동일 workload Before/After 각 3회가 끝나기 전에는 주장하지 않는다.
