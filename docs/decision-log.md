...
- Decision: outcome 분포와 warmup/measured 반복 횟수는 승인된 runtime 입력 없이는 정하지 않는다. before 결과는 creation/delivery/end-to-end duration·throughput, per-user DB calls, log/token insert-update, harness-lifecycle PostgreSQL/Redis delta와 container resource, exact SENT/FAILED/SKIPPED, dedupe replay, upstream campus/user target equality, cross-user failure continuation과 mixed-token 사용자 내부 partial failure를 수집한다. 다른 부하가 없는 전제에서 PostgreSQL notification log/token physical write와 Redis SET도 logical write의 exact count여야 하며 under/over-count를 모두 거부한다. 현재 sample은 cold JVM이고 warmup은 외부 PostgreSQL/Redis cache만 대상으로 하며, dummy token/log/dedupe history도 sample마다 누적된다. pre-run token/log cardinality·relation bytes·Redis DBSIZE가 다른 sample은 집계를 거부한다. 실제 baseline 전에 cold-JVM 유지 또는 same-JVM 반복 모델과 동일 DB/Redis snapshot 복원 또는 fixture-only cleanup 정책을 사용자가 별도로 승인한다. 실제 실행 전 상태는 `scenario-ready / not-measured`이며 개선 성과나 이력서 수치로 집계하지 않는다.
