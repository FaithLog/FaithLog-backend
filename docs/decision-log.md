...
- Decision: DB snapshot은 exact 27-table field schema, 8개 non-empty planner key, database/address/port/postmaster identity, analyze/autoanalyze/vacuum/autovacuum count와 null-or-valid timestamp를 모두 필수로 하며 maintenance와 runtime identity drift를 거부한다. app/DB container ID·image ID·StartedAt과 PostgreSQL identity는 warmup 전, measured 직전·직후, 최종 채택 전에 동일해야 한다. runtime/resource sample은 1초 contract와 2초 maximum gap, strict timestamp order, measured 양 boundary coverage, duration 기반 minimum count를 만족해야 한다. Sampling만으로 transient activity 부재를 완전 증명할 수 없으므로 operator가 shared stack의 frontend/QA/다른 요청 부재를 확보한 뒤 runtime 필수 `EXCLUSIVE_WINDOW_CONFIRMED=true`를 명시하는 것을 추가 채택 gate로 한다.
