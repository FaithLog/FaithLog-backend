...
### 2026-07-17 - Issue #198 Isolated Snapshot-Restore Measurement Policy

- Context: additive #198 dummy token/log/dedupe state made repeated samples non-equivalent. The user approved test/scenario decisions without another approval and selected `EXPECTED_WARMUP_SAMPLES=1`, `EXPECTED_MEASURED_SAMPLES=10`, and `CUMULATIVE_STATE_STRATEGY=snapshot-restore`. PM review required the snapshot to represent the fully prepared fixture, not an empty pre-fixture database, and required Redis TTL intent to be proven without comparing elapsed live PTTL.
- Decision: use one fresh canonical fixture with `sampleKind=canonical`, capture one PostgreSQL/Redis snapshot after fixture preparation, then restore and fingerprint that snapshot before each of 1 warmup and 10 measured samples. Per-sample manifest views only change local `sampleKind` metadata and never create business rows. PostgreSQL receipt binds dump SHA-256, relevant table cardinality, and ordered-row fingerprint. Redis receipt separately binds key/value SHA-1 and TTL-intent SHA-1 over key, serialized value, and captured PTTL; restore Lua returns the exact stored metadata hash used for RESTORE. Live PTTL is not compared because elapsed milliseconds are expected.
- Decision: snapshot capture/restore is permitted only for a dedicated `faithlog-perf-198*` isolated Compose project. The orchestration parent holds the host-global and canonical Compose-project locks continuously from fixture preparation through the final sample and summary; child scripts accept only a parent-PID/project-bound lock receipt and cannot release those inherited locks. `faithlog-frontend-latest`, shared/latest/frontend/QA projects, reused report/snapshot/Redis snapshot DB namespaces, identity drift, missing/tampered TTL metadata, partial restore, or fingerprint/cardinality mismatch must fail before the next sample. Named volumes are never deleted and scripts contain no Compose down/stop/restart, Docker volume/system prune, or automatic cleanup. Every receipt/result remains `accepted=false`, `automaticAdoption=false`; first rejection is exclusive and secret-free.
- Decision: #198 actual-before는 external/shared data dump를 prerequisite로 사용하지 않는다. orchestration이 dedicated schema-empty `faithlog-perf-198*` PostgreSQL에 SHA-bound current-develop V1–V11 migration 원본을 순서대로 적용하고 deterministic disabled-login synthetic ACTIVE user/member 1,000명을 한 번 provision한다. generated campus ID, exact count, migration/dataset digest, no copied data/credential/FCM을 secret-free seed receipt로 결속한 뒤에만 canonical dummy-token fixture로 진행한다. production/Flyway source는 변경하지 않는다.
- Decision: attempt `before-20260717-01`은 actual PostgreSQL `inet_server_addr()::text=127.0.0.1/32` 표기 때문에 seed runtime preflight에서 rejected됐고 migration/seed/fixture/snapshot/Gradle load는 0이다. actual raw evidence는 보존하되 비교에서만 IPv4 exact `/32`, IPv6 exact `/128`와 expanded/compressed loopback을 canonicalize한다. 승인 입력은 CIDR 없는 `127.0.0.1|::1`만 허용하고 non-host prefix, 다른/external host, malformed/multiple CIDR는 fail closed한다. attempt 01은 영구 비재사용이며 fresh 02로 이관한다.
- Decision: attempt `before-20260717-02`는 CIDR gate와 isolated V1–V11 synthetic seed(1,000 users/1,000 ACTIVE memberships)는 성공했지만 orchestration 재-sanitize가 승인 `PERF_REPORT_ROOT`를 child에서 제거해 canonical fixture를 repository default report 경로에 생성한 뒤 parent exact-path 검증에서 rejected됐다. snapshot/restore/Gradle sample/summary는 0이다. attempt 02의 report, repository-default fixture, containers, volumes는 read-only forensic 보존하고 영구 비재사용한다. orchestration은 report root와 1+10/snapshot-restore 승인값을 모든 child에 보존하며 모든 receipt/manifest/run/summary path를 exact root 내부로 fail closed 결속한다. 다음 실행은 fresh 03 namespace만 사용한다.
- Decision: attempt `before-20260717-03`은 seed, canonical fixture, snapshot, first warmup restore까지 성공하고 local Gradle `cleanTest testClasses` harness build 및 pre-workload artifact provenance를 생성했다. pristine isolated Redis가 아직 `SET`을 실행하지 않아 `INFO commandstats`에 `cmdstat_set`이 없었고 first warmup pre-workload evidence에서 rejected됐다. Gradle test sample/workload와 warmup business execution은 0이다. parser의 missing-counter rejection은 유지한다. 대신 dedicated #198 Redis에서 snapshot 전에 fresh batch-bound key를 정확히 한 번 SET/DEL하고 DBSIZE 0, immutable Redis identity, cumulative SET/DEL count, secret-free receipt를 결속한다. FLUSHDB/reset/config mutation은 금지하며 bootstrap은 모든 sample counter window 이전에 정확히 한 번만 실행한다. attempt 03은 read-only·비재사용 보존하고 fresh 04로 이관한다.
- Decision: attempt `before-20260717-04`는 seed/fixture/Redis bootstrap/snapshot/first warmup restore와 harness build/provenance를 통과했고 deterministic fake/local warmup business sample도 `scenarioFailureCount=0`, `externalFcmUsed=false`로 실행됐다. 그러나 fresh PostgreSQL의 legal `pg_stat_database.stats_reset=null`을 verifier가 timestamp-only로 거부해 measured sample과 summary는 0이다. warmup 결과는 rejected diagnostic-only이며 성능 claim이 아니다. PostgreSQL 및 available pg_stat_statements의 stats reset은 before/after가 모두 null이거나 동일한 유효 ISO timestamp여야 한다. null/timestamp 혼합과 timestamp drift는 계속 fail closed하며 null을 문자열/현재 시각으로 변환하지 않는다. attempt 04 evidence/container/volume은 불변·비재사용 보존하고 fresh 05로 이관한다.
- Decision: isolated fresh-05 `before-20260717-05`는 seed/fixture/snapshot/restore 11회, warmup 1회, measured 10회와 final summary를 exit 0으로 완료했다. 결과는 `conditional-isolated-snapshot-restored`, `accepted=false`, `automaticAdoption=false`, fake/local, `externalFcmUsed=false`, cold-JVM이며 자동 이력서 성과나 운영 SLO가 아니다. 1,000명 creation prepared statements 2,000은 사용자별 sendable-token select 1,000 + identity log insert 1,000으로 확인됐다. 최소 production 최적화는 manual/automatic request의 token 존재 여부를 request-wide bulk select 1회로 바꾸되 log insert, Redis dedupe key/TTL, dispatch, delivery의 per-log/per-token 독립 transaction, retry/backoff, partial failure와 token deactivate 의미를 유지한다. 예상 creation prepared statements는 약 1,001이며 실제 after 결과는 별도 PM load 전까지 주장하지 않는다. API/권한/Entity/DB/Flyway/의존성은 변경하지 않는다.
- Impact: #198 scenario/tooling/docs와 `NotificationRequestCommandService`의 token lookup implementation만 변경한다. API, authorization, response/error, transaction annotation, Entity, DB/Flyway, dependency, default Compose, delivery partial-durability semantics는 변경하지 않는다. Fresh-05 before evidence는 불변 보존하며 이 개발 세션은 actual Docker/DB/k6/Gradle performance workload나 after measurement를 실행하지 않는다.
...
- Decision: #198 workload provenance는 사용하지 않는 deployed app image가 아니라 attached clean worktree의 local Gradle test-profile harness에 귀속한다. runtime 승인 HEAD/contract digest, exact `origin/develop`/merge-base `6796ed146244d8f3f5b5dd7048ebe16865084a97`, src/main diff 0, source/harness/Gradle 개별 blob identity를 pre-lock부터 고정한다. build 뒤 main classes/resources와 notification test/support classes/resources 전체 artifact digest를 workload 전/후/final 비교하며 app image는 N/A다. optional `PERF_REPORT_ROOT`는 허용하지만 fixture/run namespace exclusive-create 계약은 유지한다.
...
### 2026-07-14 - Issue #198 Notification Batch Before Scenario Only

- Context: Issue #198의 production batch 최적화 전에 1,000명 알림 생성·전송의 현재 token 조회와 상태 저장 비용을 재현 가능하게 측정해야 하지만, 이 개발 세션에서는 notification job, fixture, Docker, 성능 DB, Firebase 전송과 실제 baseline 실행이 금지되었다.
- Decision: 이번 범위는 production scheduler가 same-campus ACTIVE target ID를 계산해 `NotificationRequestCommandService.requestAutomaticNotification`에 넘긴 이후와 `NotificationDeliveryWorker.processRequest`의 현재 흐름을 쓰는 test-only before harness, fixture 준비 SQL/shell, verifier/summarizer, 계약 테스트, README만 작성한다. scheduler target discovery와 cron cadence는 duration에서 제외하고 production Java/API/권한/응답/오류/트랜잭션/Entity/DB/Flyway/의존성 및 batch size 후보는 변경하지 않는다.
- Decision: 실제 Firebase는 사용하지 않는다. 사용자가 runtime에 명시한 dedicated Compose project와 실제 PostgreSQL/Redis label·published port를 일치시키고 local profile, deterministic fake FCM sender, production retry backoff, context-startup seed runner mock, `ddl-auto=validate`, credential 부재를 강제한다. `faithlog-latest`, 운영/shared profile, endpoint 불일치, label 누락, manifest의 Compose project/database 불일치, dirty worktree, #198 host-global lock 충돌은 fail closed한다. fixture는 기존 1,000명 PERFORMANCE dataset에서 fixtureRunId별 `PERFORMANCE_198_DUMMY:` token과 marker log만 허용하고 실제 token/credential을 기록하지 않는다. 다른 기존 QA/load entrypoint는 같은 lock을 사용하지 않으므로 실제 실행 전 operator가 병렬 부하 부재를 별도 확인한다.
- Decision: outcome 분포와 warmup/measured 반복 횟수는 승인된 runtime 입력 없이는 정하지 않는다. summarizer는 `EXPECTED_WARMUP_SAMPLES`와 `EXPECTED_MEASURED_SAMPLES`를 필수로 받아 exact sampleKind count와 전체 unique fixtureRunId를 검증하며 warmup 0과 measured 1을 거부한다. additive fixture가 sample마다 token/log/dedupe history를 늘리므로 `CUMULATIVE_STATE_STRATEGY=snapshot-restore|fixture-only-cleanup`도 runtime 필수 enum이지만, 어느 전략도 아직 승인·구현되지 않아 summarizer는 count 검증 뒤 항상 non-zero로 종료하고 percentile 결과를 만들지 않는다. cold-JVM 유지 또는 same-JVM 반복 모델과 cumulative-state 전략은 실제 baseline 전에 사용자가 별도로 승인한다.
- Decision: fixture prep과 runner는 #198 global lock과 실제 Compose project 이름을 쓰는 canonical `/tmp/faithlog-performance-${actualComposeProject}.lock`을 함께 획득하며 canonical lock 충돌 시 SQL/Gradle을 0건으로 유지한다. runner는 initial/before/after/final의 container ID/image/start time/Compose service·config hash, PostgreSQL database/endpoint/postmaster start, Redis run ID/port/uptime를 비교하고 verifier 전에 runtime 교체를 거부한다. PostgreSQL/Redis snapshot은 exact schema·stats reset·captured ordering·strict numeric monotonicity를, Docker evidence는 approved cadence의 PostgreSQL+Redis immutable identity pair 최소 2시점과 lifecycle coverage를 요구한다. missing/null/string/mixed-container/one-sample evidence는 모두 실패한다.
- Decision: before 결과는 creation/delivery/end-to-end duration·throughput, per-user DB calls, log/token insert-update, harness-lifecycle PostgreSQL/Redis delta와 container resource, exact SENT/FAILED/SKIPPED, dedupe replay, upstream campus/user target equality, cross-user failure continuation과 mixed-token 사용자 내부 partial failure를 수집할 준비만 한다. 다른 부하가 없는 전제에서 PostgreSQL notification log/token physical write와 Redis SET도 logical write의 exact count여야 하며 under/over-count를 모두 거부한다. 실제 실행 전 상태는 `scenario-ready / not-measured`이며 개선 성과나 이력서 수치로 집계하지 않는다.
- Decision: PM 재리뷰 finding에 따라 pre-lock 조회값은 canonical project lock discovery에만 사용하고, lock 직후 published port를 포함한 container/server identity를 다시 캡처해 fixture SQL과 runner endpoint의 유일한 truth로 사용한다. locked/initial/before/after/final 연속성, immutable-ID Docker stats target, missing Redis `cmdstat_set` fail-closed, HUP/INT/TERM sampler·lock cleanup을 계약으로 고정한다. dummy token 판별은 wildcard `LIKE`가 아닌 exact `PERFORMANCE_198_DUMMY:` prefix만 사용해 near-prefix 실제 token mutation을 금지한다. Docker sample maximum gap은 임의 5초 tolerance를 두지 않고 `PERF_DOCKER_STATS_MAX_GAP_MILLISECONDS` runtime 필수 승인값으로 남긴다.
- Decision: 재리뷰 runtime finding에 따라 `locked,initial,before` identity는 Gradle/DB mutation 전에 즉시 검증하고 final phase 목록은 inherited 환경값과 무관하게 exact 고정한다. fixture SQL과 모든 PostgreSQL/Redis evidence query도 locked immutable container ID만 대상으로 한다. fixture prep/runner 모두 lock 획득 전에 signal-safe cleanup을 설치하고 ownership flag가 true인 lock만 해제해 선점된 canonical lock을 보존한다.
- Impact: TDD RED/GREEN과 비Docker 검증·문서·Obsidian·작업 단위 커밋까지만 개발 세션에서 완료한다. dedicated runtime, fixture 분포, 반복 횟수와 baseline 실행은 PM의 별도 승인 대상으로 남고 push/PR/merge는 수행하지 않는다.
