...
- Decision: If `BillingService` remains for compatibility, keep it as a repository-free, transaction-free thin delegate that does not own business rules. The current branch's mechanical relocation of legacy payment-account reads is under PM review and is not recorded here as a separate user-approved Issue #149 decision.
