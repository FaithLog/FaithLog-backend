...
- Decision: stable `datasetId`와 실행별 immutable `fixtureRunId`를 분리하고, primary campus ACTIVE 1,000명과 isolation campus ACTIVE 50명을 새 row로만 생성한다. Prayer는 40조×25명과 제출 800/미제출 200, Poll은 5 options와 응답 800/미응답 200, comments 200, templates 40×8을 고정한다. 기존 row와 다른 fixture run의 row는 수정·삭제하지 않는다. 종료 2/5/8일 visibility fixture는 현재 run에서 API로 생성·마감한 Poll의 정확한 `id + campus_id + title` 소유권을 모두 확인한 경우에만 시각을 보정한다.
