...
- Decision: 투표 목록과 상세 응답에 `createdByUserId`와 `manageableByMe`를 추가한다. COFFEE 투표의 `manageableByMe`는 현재 요청자가 ACTIVE COFFEE 담당자이면서 해당 투표 생성자일 때만 true이며, 다른 담당자와 비담당 관리자는 읽기는 유지하되 false다. 커피 외 투표는 기존 관리자 command 권한과 동일하게 계산하고, 프론트는 역할을 재조합하지 않고 이 필드로 마감·정산 등 write 버튼을 제어한다.
