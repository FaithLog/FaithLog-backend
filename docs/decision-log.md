...
- Impact: 최초 시나리오 준비 단계의 Docker/DB/load 0건 기록은 당시 상태다. 이후 PM exclusive actual의 `g01`/`g02` 등 모든 evidence와 ID는 삭제·수정·재사용하지 않으며 `g02` partial은 rejected evidence일 뿐 valid baseline이 아니다. 현재도 `scenario-ready / not-measured`이고 production Java/API/권한/응답/오류/트랜잭션/Entity/DB/Flyway/의존성 변경과 개선 성과 수치는 없다.
