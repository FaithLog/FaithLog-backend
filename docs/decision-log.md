...
- Decision: Prayer current season/groups/assignable/admin weekly board/member weekly board, Poll member list/detail/results/comments/isolation, Poll admin common list/detail/results/comments/missing-members/templates/isolation을 endpoint별 독립 k6 프로세스로 측정한다. 상위 runner는 `prayer -> poll-member -> poll-admin`을 순차 실행하고 global lock과 다른 k6 부재를 확인하므로 Prayer/Poll 부하를 한 구간에 섞거나 다른 부하와 병렬 실행하지 않는다.
- Decision: 각 endpoint는 custom Trend p50/p95/p99/max, Counter request count/throughput, Rate failure, app/PostgreSQL CPU/RAM, Hibernate SQL query count/queriesPerRequest/repeated normalized SQL, PostgreSQL table estimated row/scan/fetch/write counter, 실제 app image tag/immutable image ID와 published target port 및 Compose project/service/config-hash label을 기록한다. 목록 개수·정렬·editable/myGroup·campus isolation 및 Poll visibility/status/time-window/results correctness failure는 0건이어야 한다. runner는 각 endpoint 직전·직후에 fixture time window를 다시 검증한다. k6/resource sampler/time-window/log/after-DB-snapshot 실패, 필수 latency/throughput/table/resource evidence 누락·오형식 또는 read-path write delta가 있는 report는 reason과 함께 `accepted=false`/`measurementStatus=rejected`로 남기고 runner를 비정상 종료하며 baseline으로 인정하지 않는다.
