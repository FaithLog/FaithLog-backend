...
- Actual checkpoint: `PERF_1000_20260716_195_A` provisioning과 `ISSUE195_BEFORE_20260716_A` fixture manifest는 성공 상태로 보존한다. `EXEC195_BEFORE_20260716_A`는 첫 warmup 초기화에서 k6가 JSON을 JavaScript module로 파싱해 exit `107`이 되었고 warmup/measured HTTP는 모두 0이므로 execution/report 전체를 재사용하지 않는다. JSON contract의 내용·hash·권한·시나리오 의미는 유지하고 k6 init context에서 `open()` 후 `JSON.parse`한다. 다음 actual load는 preserved dataset/fixture와 fresh `EXEC195_BEFORE_20260716_B`만 사용하며, PM trap의 임시 ADMIN→USER 원복을 먼저 확인한다.
