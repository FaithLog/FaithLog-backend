...
- Decision: 실제 Firebase는 사용하지 않는다. 사용자가 runtime에 명시한 dedicated Compose project와 실제 PostgreSQL/Redis label·published port를 일치시키고 local profile, deterministic fake FCM sender, production retry backoff, `ddl-auto=validate`, credential 부재를 강제한다. `faithlog-latest`, 운영/shared profile, endpoint 불일치, label 누락, manifest의 Compose project/database 불일치, 다른 부하 lock 충돌은 fail closed한다. fixture는 기존 1,000명 PERFORMANCE dataset에서 fixtureRunId별 `PERFORMANCE_198_DUMMY:` token과 marker log만 허용하고 실제 token/credential을 기록하지 않는다.
- Decision: outcome 분포와 warmup/measured 반복 횟수는 승인된 runtime 입력 없이는 정하지 않는다. before 결과는 creation/delivery/end-to-end duration·throughput, per-user DB calls, log/token insert-update, PostgreSQL/Redis delta, Java/container resource, exact SENT/FAILED/SKIPPED, dedupe replay, upstream campus/user target equality, permanent failure 이후 후속 success continuation을 수집한다. 실제 실행 전 상태는 `scenario-ready / not-measured`이며 개선 성과나 이력서 수치로 집계하지 않는다.
