...
### 2026-07-16 - Issue #197 Current-develop Baseline Preparation And Exclusive Measurement Policy

- Context: Issue #197 scenario preparation resumed after #200/#201/#202/#206 landed on `origin/develop` exact HEAD `6796ed146244d8f3f5b5dd7048ebe16865084a97`. Another performance issue may own the actual measurement slot, so scenario development and load execution must not overlap.
- Decision: Keep a two-session/one-load workflow. The preparation session may change only issue-local k6, fixture, validator, fake/static correctness tests, and documentation. It must not run Docker, DB, HTTP, k6, cleanup, or scheduler work and must not modify production backend, dependencies, or Flyway. One later PM-controlled measurement session owns the common performance lock and runs scenarios sequentially. Runtime revision, app/DB/Redis image/full container identity, app JAR/API-contract digest, Compose project/service, applied Flyway identity, Redis run identity, explicit numeric-loopback app/DB/Redis targets, workload/JWT, three-role resource sampling, DB session/maintenance, BigInt counters, and `pg_stat_statements` state are required runtime evidence with no guessed defaults. Candidate retention SQL must run in an explicit read-only transaction. The first failure must be preserved in a fresh machine-readable rejection artifact with `automaticAdoption=false`.
- Decision: #201 one-month default archive visibility is independent of annual retention candidate selection. Annual charge retention uses prior-year `created_at` and terminal `PAID`/`WAIVED`/`CANCELED` status only. #200 stale duty and soft-deleted-account UNPAID rows remain preserved and are never retention candidates. #202 V11 Supabase RLS is not treated as evidence of JDBC backend correctness; endpoint and direct-JDBC evidence remain mandatory. Retention stays dry verification only until a safe cleanup trigger is separately approved.
- Impact: The Issue #197 baseline is `scenario-ready / not-measured`; no before/after performance number or independent optimization contribution is claimed. Individual branch Docker builds and after runs remain deferred to the later PM integration workflow. Index/Flyway work remains outside #197 and is not added here.

### 2026-07-16 - Issue #197 Source/Image Provenance And Fresh Evidence Namespace

- Context: The approved current-develop app image has no OCI revision/API-contract label. Read-only audit instead established a clean detached source checkout at exact revision `6796ed146244d8f3f5b5dd7048ebe16865084a97`, the Compose working directory, and image creation after the worktree HEAD reflog time. Fake runner suites also need independent report roots while production evidence must never reuse a fixture namespace.
...
### 2026-07-17 - Issue #197 Single-path Fixture Preparation And Conditional Before Evidence

- Context: The baseline runner required a fresh 1,002-user fixture, runtime credentials, and a pre-approved exact DB activity signature, but no safe preparation path existed. Exact DB-wide commit/query counts cannot be known before a fresh fixture run and may include observer, authentication, or other backend activity on a shared stack.
- Decision: Use one preparation path only: read-only namespace-zero check, exclusive local report/secret reservation, create-only public/admin API seed for warmup 1 + measured 1,000 + rollback 1, final login/token manifest, exact DB preflight, and installed k6 inspect. The runtime-only input and credentials are mode 600; raw password/access/refresh token/Authorization values are never written to evidence, logs, or argv. Partial fixtures are never cleaned up or reused automatically. Calibration datasets, duplicate workloads, proportional warmup estimation, guessed tolerances, and counter subtraction are prohibited.
- Decision: Keep HTTP/custom Counter/failure/correctness and business rows exact: measured weekly 1,000, daily 7,000, charge 1,000, rollback persisted 0. Treat `pg_stat_database`, `pg_stat_user_tables`, and optional `pg_stat_statements` as runtime-observed supporting evidence. The initial preparation contract required exact target inserts and zero unrelated table writes, but the later Fresh H decision below supersedes that cumulative-counter interpretation after actual asynchronous publication evidence. Do not require a pre-run exact activity signature. A clean shared-stack result is always `conditional-not-adoptable` with `automaticAdoption=false`.
...
### 2026-07-17 - Issue #197 Three-Role Resource Snapshot Sampler

- Context: Fresh E prepared successfully and completed all 1,000 measured requests with zero HTTP/check/custom failures. The resource gate rejected the run at stage `measured`: six raw rows showed that three separate blocking `docker stats --no-stream` calls captured app, database, and Redis about two seconds apart, so the same-role cadence exceeded the approved two-second maximum. E latency/throughput values are diagnostic-only and cannot be adopted as a baseline. Its report/fixture remains read-only, credentials were removed, the temporary ADMIN USER was restored, and cleanup/reuse is prohibited.
- Decision: Replace serial per-role sampling with exact three-role snapshots. One pre-window `docker stats --no-stream --no-trunc` call captures all approved immutable full IDs together. One continuous multi-container `docker stats --no-trunc` process covers the measured window without repeated startup delay. Every snapshot has exactly app/database/Redis once with one shared timestamp; snapshot timestamps are strictly increasing. A stop marker requires one complete post-marker final boundary snapshot before normal exit. Identity mismatch, duplicate/missing/extra rows, malformed CPU/RAM, premature EOF, incomplete snapshot, stalled stream beyond the unchanged runtime max-gap deadline, or sampler failure remains fail-closed and non-zero.
- Impact: E is permanently non-adoptable. Fresh F (`PERFORMANCE_1000_20260717_DEVOTION_197_F` / `ISSUE197_20260717_DEVOTION_BEFORE_F`) is the only next candidate. This development session changes scenario tooling/tests/docs only and does not run production, Flyway, Docker, DB, API, k6 load, retention, or cleanup.

### 2026-07-17 - Issue #197 Docker Stats Exact ANSI Screen Framing

- Context: Fresh F prepared successfully and completed 1,000 measured requests with zero failures, but the resource sampler exited before measured end because installed Docker streaming output framed the first full ID with CSI cursor-home `ESC[H` and ended every row with CSI erase-to-EOL `ESC[K`. The no-stream output was control-free. Field choice was not the root cause. F latency/throughput is diagnostic-only and cannot be adopted; its evidence/fixture is read-only, credentials were removed, the temporary ADMIN USER was restored, and cleanup/reuse is prohibited.
- Decision: Format Docker stats identity explicitly with `.ID`. Before strict ID/CPU/memory parsing, accept only either a fully control-free three-row snapshot or the exact installed streaming screen framing: cursor-home only at the beginning of row 0 and erase-to-EOL at the end of all three rows. Strip only those exact controls. Unknown/different/malformed CSI, mid-field or remaining control bytes, mixed framing, and surrounding text remain fail-closed. Marker, final boundary, max-gap deadline, identity, and all workload/adoption thresholds are unchanged.
- Impact: F is permanently non-adoptable. Fresh G (`PERFORMANCE_1000_20260717_DEVOTION_197_G` / `ISSUE197_20260717_DEVOTION_BEFORE_G`) is the only next candidate. A read-only installed Docker smoke parsed one current app/PostgreSQL/Redis streaming snapshot into the exact three approved full IDs without Docker lifecycle, DB, API, or k6 load mutation.

### 2026-07-17 - Issue #197 Docker Stats Recurring Display Protocol

- Context: Fresh G prepared successfully and completed 1,000 measured requests with zero failures, but the sampler stopped before the final boundary. The installed Docker stream inserted one standalone `ESC[K` physical line between complete snapshots and prefixed each recurring first row with `ESC[JESC[H`; the previous parser grouped every three physical lines and treated that separator as the next data row. G latency/throughput is diagnostic-only and cannot be adopted; its evidence/fixture remains read-only, credentials were removed, the temporary ADMIN USER was restored, and cleanup/reuse is prohibited.
- Decision: Parse streaming Docker display output as an exact state machine. The initial first data row requires `ESC[H`; all three data rows require an `ESC[K` suffix. Every later snapshot requires exactly one standalone `ESC[K` separator followed by a first data row prefixed exactly `ESC[JESC[H`, then two suffixed data rows. Missing, extra, or out-of-order separator/prefix, `ESC[2J`, unknown/mid-field control, malformed framing, incomplete snapshots, and identity/schema mismatch remain fail-closed. The control-free no-stream parser, marker/final-boundary behavior, stall deadline, and 1-second/2-second cadence are unchanged.
- Impact: G is permanently non-adoptable. Fresh H (`PERFORMANCE_1000_20260717_DEVOTION_197_H` / `ISSUE197_20260717_DEVOTION_BEFORE_H`) is the only next candidate. A read-only installed Docker smoke sent three complete recurring snapshots through `stream-samples`, created the stop marker after two snapshots, captured the final boundary, and validated nine exact-ID rows with 0.497/0.501-second gaps. No Docker lifecycle, DB, API, HTTP, or k6 load mutation occurred.

### 2026-07-17 - Issue #197 Direct Cardinality And Asynchronous DB Counter Classification

- Context: Fresh H completed preparation, warmup 1, and measured 1,000 with zero HTTP/check/custom failures. Direct read-only joins proved exact application state for campus 82/week `2026-07-27`: weekly 1,000 users, daily 7,000 rows for 1,000 users, and 1,000 PENALTY/DEVOTION_RECORD charges bound by weekly source and user, each 2,250 and totaling 2,250,000. The success campus had exactly 1,001 devotion charges including warmup. PostgreSQL cumulative target-table inserts were published only as 595/4,165/595 during the snapshot while unrelated users and other-database counters also advanced. The previous exact cumulative gate rejected H at measured before rollback.
- Decision: Make fixture-owned direct SQL the exact acceptance source for measured weekly/daily/charge cardinality, distinct users, exact week dates, amount sum, source/week/user binding, uniqueness, success-campus warmup+measured isolation, and final rollback persisted zero. Capture it after measured DB counters and before rollback, with immutable app/PostgreSQL/Redis identity immediately before and after. Treat all PostgreSQL cumulative table/database/pgss deltas as lossless monotonic supporting evidence only and record positive deltas as source-unattributed. Do not sleep, tolerate, subtract, estimate background work, or infer publication ratios. Malformed/regressing counters, reset, planner/maintenance drift, external sessions, and runtime replacement remain fail-closed.
- Impact: H is permanently rejected/non-reusable and its metrics are diagnostic-only. Shared unrelated positive deltas no longer reject exact fixture-owned correctness, but every result remains `conditional-not-adoptable` with `automaticAdoption=false`. Fresh I (`PERFORMANCE_1000_20260717_DEVOTION_197_I` / `ISSUE197_20260717_DEVOTION_BEFORE_I`) is the only next candidate. No production, Flyway, dependency, Docker lifecycle, DB/API write, HTTP, k6 load, scheduler, or cleanup change is made in this development session.

### 2026-07-17 - Issue #197 Rollback Fixture Must Reach The Missing-Account Failure Point

- Context: Fresh I completed preparation, warmup 1, measured 1,000, measured direct cardinality, and runtime continuity. Its rollback request returned a success response instead of `BILLING_REQUIRED_PAYMENT_ACCOUNT_MISSING`, so k6 rejected it at rollback and final persisted-row verification was not reached. Current develop calculates the fine from the target campus's active rules and returns successfully before payment-account lookup when the amount is zero. I had zero rollback-campus accounts as intended, but prepare created all four rules only in the success campus, so the rollback amount was zero and the missing-account branch was unreachable.
- Decision: Treat I as a scenario fixture bug, not a production defect. Create the same approved four active rules in both fresh campuses while keeping exactly one active PENALTY account only in the success campus and zero in the rollback campus. Preflight must separately prove each campus has the exact four rule types/calculation pairs and the same positive `expectedPenaltyAmount`. The unchanged rollback request then writes weekly/daily/submitted state inside the transaction, reaches charge creation, fails on the absent rollback-campus account, and final direct SQL must still prove weekly/daily/charge persisted zero. Do not weaken the error response or persistence contract.
- Impact: Prepare changes from 3,014 requests/rule 4 to 3,018 requests/rule 8. I and its diagnostic metrics are permanently rejected/non-reusable; its final exact DB gate was not captured in the run. Fresh J (`PERFORMANCE_1000_20260717_DEVOTION_197_J` / `ISSUE197_20260717_DEVOTION_BEFORE_J`) is the only next candidate. This development session does not run Docker, DB, API, HTTP, k6, cleanup, or production code.

### 2026-07-17 - Issue #197 Conditional Before And Weekly Daily Bulk Load

- Context: Fresh J (`PERFORMANCE_1000_20260717_DEVOTION_197_J` / `ISSUE197_20260717_DEVOTION_BEFORE_J`) completed warmup 1, measured 1,000, exact direct cardinality, and the missing-account rollback proof with zero measured failures. It is a valid shared-stack before but remains `conditional-not-adoptable` with `automaticAdoption=false`; cumulative PostgreSQL counters have asynchronous publication and are supporting-only.
- Decision: Derive the optimization target from the current Java call graph, not cumulative counter deltas. A weekly save/submit previously performed seven `findByWeeklyRecordIdAndRecordDate` calls and one `findByWeeklyRecordIdOrderByRecordDateAsc` call for the same seven daily rows. Load the week's daily rows once, index them by `recordDate`, update existing entities or create missing entities in memory, pass only missing rows through one `saveAll`, and calculate the weekly summary from that same complete list.
