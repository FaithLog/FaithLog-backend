...
- #194 PostgreSQL 실행계획 기반 index before-baseline 시나리오 준비:
  ...
  - TDD/검증: 최초 production 변경 전 Node 계약은 `5 tests / 5 failures` RED 뒤 `5 tests / 0 failures` GREEN이었다. PM finding 계약은 기존 5개를 보존한 채 `11 tests / 5 pass / 6 fail` RED였고 최소 수정 후 `11 tests / 11 pass` GREEN이다. Node/Bash syntax, JSON 24-query parse, SQL read-only safety, `git diff --check`가 성공했다. 첫 전체 Gradle은 macOS Byte Buddy dynamic attach 구간에서 장시간 정지해 해당 실행만 중단했고, 현재 resolved `byte-buddy-agent 1.17.8`을 test JVM에 선로딩해 `449 tests / 0 failures / 0 errors / 3 skipped`, build, asciidoctor를 성공했다. 실제 성능 수치와 이력서 개선 문장은 exact production SQL capture, baseline 실행, 별도 index 승인 전까지 pending이다.
