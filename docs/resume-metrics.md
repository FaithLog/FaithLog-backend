...
- 초기 #236 단독 gate 뒤 최종 통합에서는 poll 참여 shape를 제거하고 comment/penalty 및 V17 archive 계약을 반영해 다시 검증했다. 최종 exact-HEAD `./gradlew clean test build asciidoctor`는 `836 tests / failures 0 / errors 0 / skipped 14`와 REST Docs HTML 생성을 확인했다. 이 실행 시간은 제품 성능 수치가 아니다.
- 최종 순서는 #237 V14, corrected #236 V15, #238 V16, compact archive V17, cleanup retry/lease V18이다. V15 SQL SHA-256은 `bd7b956e8aba48d9c21dd0cd113cb09170aed41023002318aa723489d12dfb34`이며, 전용 PostgreSQL 17에서 clean V1→V18과 보호된 upgrade 8건의 RLS/FK/CHECK/order 계약을 확인했다.
...
- V18 adds durable cleanup attempt/backoff/lease metadata. Executable service and repository tests prove 100 failed head candidates do not starve candidate 101, transient failures retry, backoff caps at 24 hours, and an expired multi-instance claim is recoverable. These are correctness and storage-retention improvements, not measured latency or cost-savings claims.
- Final whole-diff review found that the annual archive writer could pass a full 1,000-member year's source IDs through one `IN` predicate. A test-first correction bounds lookup and write batches to 500 while preserving idempotency and the archive/delete transaction. The final exact-HEAD clean gate is 836 tests / failures 0 / errors 0 / skipped 14 with build and Asciidoctor success; a disposable PostgreSQL 17 gate passed all 8 clean/upgrade migration cases without touching the shared QA database.
