...
- #148 Billing 계좌와 청구 명령 책임 분리:
  ...
  - 호환 경계: Controller와 Devotion/Poll adapter는 전용 서비스로 직접 연결했다. 기존 `BillingService`는 3개 명령 서비스와 `BillingQueryService`만 delegate하는 repository/transaction/business-rule-free façade로 제한했다. 현재 브랜치는 #149 최적화·응답 변경 없이 계좌 조회 3개 surface를 기존 `BillingQueryService`로 1:1 이동했으며, 이 기계적 호환 이동은 PM 최종 판단 전 별도 사용자 승인 결정으로 단정하지 않는다.
