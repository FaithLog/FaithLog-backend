...
- #198 알림 생성·전송 before 시나리오 준비 (`scenario-ready / not-measured`):
  ...
  - additive fixture는 sample마다 dummy token/log/dedupe history를 늘리므로 현재 summarizer는 fail-closed disabled다. runtime의 approved exact warmup/measured count와 unique fixtureRunId를 먼저 검증하고 warmup 0/measured 1을 거부하지만, `snapshot-restore|fixture-only-cleanup` 중 사용자 승인·구현된 cumulative-state 전략이 아직 없어 summary/percentile을 만들지 않는다. 실제 fixture/job/Docker/성능 DB/baseline은 실행하지 않았으므로 수치와 개선 성과는 `not measured`다.
