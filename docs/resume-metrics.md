...
- ACTIVE 캠퍼스 여정, 경건 완료·연속일·활동 월, 기도 distinct 제출 주차·시즌, 본인의 삭제되지 않은 댓글 작성 수, 본인 경건 벌금의 PAID/UNPAID count·amount를 집계한다. 투표 참여/PollType 통계와 기도문, 투표 선택·메모·댓글 본문, 이메일, 계좌 정보, 개별 청구, JWT/session/device/FCM token은 저장하거나 반환하지 않는다. 다중 캠퍼스의 같은 날짜와 같은 달력 주차는 한 번만 집계하고 기도 시즌 수는 독립적으로 유지한다.
...
- 초기 #236 단독 gate 뒤 최종 통합에서는 poll 참여 shape를 제거하고 comment/penalty 및 V17 archive 계약을 반영해 다시 검증했다. 최종 exact-HEAD `./gradlew clean test build asciidoctor`는 `843 tests / failures 0 / errors 0 / skipped 15`와 REST Docs HTML 생성을 확인했다. 이 실행 시간은 제품 성능 수치가 아니다.
- 최종 순서는 #237 V14, corrected #236 V15, #238 V16, compact archive V17, cleanup retry/lease V18이다. V15 SQL SHA-256은 `bd7b956e8aba48d9c21dd0cd113cb09170aed41023002318aa723489d12dfb34`이며, 전용 PostgreSQL 17에서 clean V1→V18과 보호된 upgrade 9건의 RLS/FK/CHECK/order 계약을 확인했다.
...
- Fresh provisioning/fixture TDD: #192 dataset의 `999 USER + 1 ADMIN` 구성은 #195의 exact `1,000 ACTIVE USER`, admin-excluded 계약에 맞지 않아 재사용을 금지했다. Fresh namespace/report reservation, public signup-only 1,000명, collision/partial/role/active/duplicate/credential/3-container continuity 계약을 test-only RED로 고정한 뒤 issue-local provisioner를 구현했다. 추가 PM findings에서 30분 access token을 2,000여 요청에 재사용하는 provisioning/fixture 경계, report overwrite, Docker child credential 상속을 RED로 재현하고 request별 runtime `exp` margin, exclusive receipt, child allowlist로 보정했다. 후속 strict target 묶음은 `10 assertions / 7 pass / 3 fail` RED로 absolute report root, numeric loopback, full immutable identity를 고정한 뒤 GREEN으로 전환했다. 마지막 handoff 대조에서 runner fallback과 provisioner 상대경로 허용을 실행형 RED로 재현하고 provisioning/fixture/execution 모두 동일한 absolute runtime root에서만 시작하도록 보정했다. Actual 실행 전 tooling 검증은 fixture `10/10`, provisioning `19/19`, scenario `44/44`, 전체 issue-local `73/73` GREEN이었다.
- Actual-before blocker: PM slot에서 fresh dataset `PERF_1000_20260716_195_A`의 ACTIVE USER 1,000명 provisioning과 `ISSUE195_BEFORE_20260716_A`의 campuses 25/memberships 1,025/duties 101 fixture가 성공했다. 첫 warmup은 k6가 imported JSON을 JavaScript로 해석해 exit `107`로 초기화 실패했고 warmup/measured HTTP 요청은 모두 0이었다. Execution A/report는 폐기·재사용 금지이며 성공한 dataset/fixture는 그대로 보존한다. 설치된 k6 inspect로 같은 실패를 RED 재현한 뒤 JSON contract의 내용과 의미를 바꾸지 않고 init-context `open()` + `JSON.parse`로 GREEN 전환했다. 이 시점의 fixture `10/10`, provisioning `19/19`, scenario `45/45`, 전체 `74/74`였으며 이후 B의 rejected 결과는 다음 항목에 기록한다.
- Actual-before B blocker: fresh B의 첫 warmup은 142 HTTP/checks 1,278/1,278/HTTP·custom failure 0으로 끝났지만, k6 v2 Rate가 `{value:0,passes:0,fails:142}`를 내보내 기존 validator가 누락된 `rate` 필드를 요구해 거부됐다. Measured HTTP는 0이고 B warmup latency는 baseline/성과로 채택하지 않으며 B report는 read-only/non-reusable다. 보존 summary를 직접 읽어 content/size/mtime 불변을 확인하는 RED 뒤, exact `value=0/passes=0/fails=requestCount`와 direct/values, warmup/measured, normalized DB integrity를 모두 fail-closed 검증했다. Final fixture `10/10`, provisioning `19/19`, scenario `46/46`, 전체 `75/75`; 상태는 `scenario-ready/not-measured`, `automaticAdoption=false`다. PM은 service role 0 ADMIN/1 USER 원복을 DB 확인했고 다음은 preserved dataset/fixture + fresh `EXEC195_BEFORE_20260716_C`만 사용한다.
- Actual-before C blocker: fresh C의 첫 `admin_users/first_page` warmup은 530 HTTP/checks 4,770/4,770/HTTP·custom failure 0이었지만, measured-before DB evidence 뒤 첫 resource snapshot에서 Docker의 rounded decimal binary memory display를 integer byte로 요구해 거부됐다. Measured HTTP는 0이고 warmup latency는 baseline/성과로 채택하지 않으며 C report는 read-only/non-reusable다. 실제 app/PG/Redis 문자열과 모든 지원 단위·safe boundary를 nearest whole byte capture부터 resource adoption까지 RED→GREEN했고, future k6 setup data에서 token을 제거해 installed-k6 no-HTTP `handleSummary` serialization에도 sentinel이 없음을 고정했다. Final fixture `10/10`, provisioning `19/19`, scenario `49/49`, 전체 `78/78`; 상태는 `scenario-ready/not-measured`, `automaticAdoption=false`다. PM은 broad `perf-195` 임시 계정 service role 0 ADMIN/3 USER를 확인했고 다음은 preserved dataset/fixture + fresh `EXEC195_BEFORE_20260716_D`만 사용한다.
- Actual-before D blocker: fresh D의 첫 `admin_users/first_page` warmup 542건과 measured 7,197건/checks 64,773/64,773/failure 0은 성공했지만 after table artifact에 psql meta-command status가 섞여 `measured-evidence-after`에서 거부됐다. D의 latency/throughput을 포함한 모든 수치는 baseline/성과로 채택하지 않으며 D report는 read-only/non-reusable다. Preserved D와 fake psql로 table/runtime/available/unavailable query stdout 오염을 RED 재현하고 모든 collector psql 경로를 quiet machine-output mode로 통일했다. Final fixture `10/10`, provisioning `19/19`, scenario `50/50`, 전체 `79/79`; 상태는 `scenario-ready/not-measured`, `automaticAdoption=false`다. PM은 broad `perf-195` 임시 계정 service role 0 ADMIN/4 USER와 cleanup을 확인했고 다음은 preserved dataset/fixture + fresh `EXEC195_BEFORE_20260716_E`만 사용한다.
- Actual-before E blocker: fresh E의 첫 `admin_users/first_page` warmup 334건과 measured 4,200건/checks 37,800/37,800/failure 0은 성공했지만 DB-wide commit delta 8,427이 request-derived 8,400 + observer 1보다 26 커 old exact-equality gate에서 거부됐다. E의 latency/throughput을 포함한 모든 수치는 baseline/성과로 채택하지 않으며 E report는 read-only/non-reusable다. 별도 PM idle 관측도 raw 18 - observer 1 = source-unattributed background 17이어서 허용오차/차감 대신 same-duration idle control, strict full snapshot, control→measured timestamp/case/duration/BigInt self-binding, `supporting-only`/`automaticAdoption=false`를 RED→GREEN했다. Initial token gate에는 control duration도 포함해 권고값에서 최소 390초를 control write 전에 요구한다. PM 독립 전체는 `80/80` GREEN이고 2초 read-only smoke도 supporting-only/subtraction false를 확인했지만 모두 diagnostic이며 baseline이 아니다. 상태는 `scenario-ready/not-measured`; 다음 fresh F는 Issue #208 공통 audit 완료 전 실행하지 않는다.
- Actual-before F blocker: fresh F는 `admin_users/first_page`를 완료한 뒤 `admin_users/middle_page`의 `measured-evidence-after`에서 거부됐다. 해당 warmup은 HTTP 837/failure 0, measured는 12,342 requests/checks 전부 통과/failure 0, DB-wide commit `167243 -> 191857`(delta 24,614), rollback 0이었고 old `requestCount × 2 + observer = 24,685`보다 71 작았다. Same-duration immediately-before idle control은 raw commit 40/background-unattributed 39/rollback 0이었다. After snapshot은 measured-end보다 6.304초, measured max HTTP 4.117026초보다 늦어 단순 즉시 캡처만으로 부족분을 설명할 수 없었고, static source의 인증 repository transaction 1 + endpoint service transaction 1 경계와 DB-wide cumulative snapshot의 publication/source-attribution 한계를 분리했다. Preserved F 전체 파일 content/SHA-256/size/nanosecond mtime 불변 actual-fixture RED 뒤, DB-wide acceptance만 strict `requestCount + observer` lower bound로 낮추고 source `2 × requests + observer`는 diagnostic으로 유지했다. 초과 commit은 모두 source-unattributed `conditional-not-adoptable`; tolerance/background subtraction/비례추정/성과 채택 없이 `automaticAdoption=false`다. F 수치는 rejected diagnostic-only이며 resume 성과로 집계하지 않는다. 임시 계정 `USER` 원복과 lock/process cleanup을 확인했고 다음은 preserved dataset/fixture + fresh `EXEC195_BEFORE_20260716_G`만 사용한다. 최종 issue-local은 fixture `10/10` + provisioning `19/19` + scenario `53/53` = `82/82` GREEN이다.
...
- 반복 정책은 warmup 1회, measured 10회, dedicated `faithlog-perf-198*` isolated stack의 snapshot-restore로 확정했다. canonical fixture를 1회 준비한 뒤 snapshot 1회, restore/hash·cardinality 검증 11회, sample 11회 순서를 fake full-run으로 고정했다. Parent가 두 performance lock을 전체 orchestration 동안 보유해 restore/sample 간 간격도 차단한다. PostgreSQL dump/state/cardinality와 Redis key/value 및 TTL-intent hash를 receipt로 결속하며 missing/tampered TTL, partial/drift, shared stack에서 즉시 중단한다.
- 외부/shared dump prerequisite를 제거했다. schema-empty #198 isolated DB에 current-develop V1–V11을 직접 검증·적용하고 synthetic ACTIVE user/member 1,000명을 deterministic provision한 뒤 generated campus ID와 migration/dataset SHA를 secret-free receipt로 자동 전달한다. 실제 Docker/DB/load는 아직 실행하지 않았으며 상태는 계속 scenario-ready / not-measured다.
- actual attempt 01은 PostgreSQL raw inet `127.0.0.1/32` preflight 표기 불일치로 migration/seed/fixture/snapshot/Gradle sample 모두 0에서 rejected됐다. raw evidence를 보존하면서 host CIDR만 lossless 비교하는 계약으로 보정했고 attempt 01 report/container/volume은 비재사용 보존, 다음은 fresh project/batch/report `faithlog-perf-198-20260717-02` / `before-20260717-02` / `/private/tmp/faithlog-perf-198-reports/20260717-02`다. 성능 수치는 생성되지 않았다.
- actual attempt 02는 CIDR gate와 V1–V11 synthetic seed(1,000 users/1,000 ACTIVE memberships)까지 성공했으나 child `PERF_REPORT_ROOT` 누락으로 canonical fixture가 repository default report 경로에 생성되어 parent 검증에서 rejected됐다. snapshot/restore/Gradle sample/summary는 0이고 성능 수치는 없다. attempt 02의 report/default fixture/container/volume은 read-only·비재사용 보존한다. fresh 03은 project `faithlog-perf-198-20260717-03`, batch `before-20260717-03`, report `/private/tmp/faithlog-perf-198-reports/20260717-03` 및 unique container/volume names를 사용한다.
- actual attempt 03은 seed/fixture/snapshot/first warmup restore와 `cleanTest testClasses` harness build 및 pre-workload artifact provenance까지 성공했다. pristine Redis의 missing `cmdstat_set` strict evidence rejection으로 Gradle test sample/workload와 warmup business execution은 0이며 성능 수치는 없다. snapshot 전 dedicated Redis에서 batch-bound SET 1회/DEL 1회 후 DBSIZE 0과 identity/counters를 secret-free receipt로 결속하도록 보정했다. attempt 03 artifacts/containers/volumes는 read-only·비재사용 보존한다. fresh 04는 project `faithlog-perf-198-20260717-04`, batch `before-20260717-04`, report `/private/tmp/faithlog-perf-198-reports/20260717-04` 및 unique container/volume names를 사용한다.
- actual attempt 04는 seed/fixture/Redis bootstrap/snapshot/restore 01-warmup, harness build/provenance 및 deterministic fake/local warmup business execution까지 성공했다(`scenarioFailureCount=0`, `externalFcmUsed=false`). PostgreSQL `stats_reset=null` verifier rejection으로 measured 0/summary 0이며 아래 warmup 수치는 모두 rejected diagnostic-only이고 성능 claim이 아니다: creation 4393.620709ms, 227.6027145/s, DB prepared 2000, per-user 2; delivery 104792.839833ms, 7.6341094/s, DB prepared 1805, per-user 1.805; end-to-end 109186.460542ms, 9.1586447/s; final SENT 700, FAILED 100, SKIPPED 200, PENDING 0. attempt 04 reports/containers/volumes는 read-only·비재사용 보존한다. fresh 05는 project `faithlog-perf-198-20260717-05`, batch `before-20260717-05`, report `/private/tmp/faithlog-perf-198-reports/20260717-05` 및 unique container/volume names를 사용한다.
- fresh-05 conditional isolated before가 exit 0으로 완료됐다(1 warmup + 10 measured, snapshot restore 11, `accepted=false`, `automaticAdoption=false`, fake/local, external FCM 0, cold JVM). creation duration p50/p95/p99/max는 4374.0778125/4587.04813545/4616.35986069/4623.687792ms, throughput은 228.62278901445/235.99378014695/238.75170402809/239.44118499837/s, prepared statements 2000, per-user 2다. delivery duration은 104736.676979/105108.45254375/105224.59380875/105253.629125ms, throughput은 7.63820300717/7.64814053088/7.64826889765/7.64830098934/s, prepared statements 1805, per-user 1.805다. end-to-end duration은 109151.666791/109580.61548765/109694.45399753/109722.913625ms, throughput은 9.16156855108/9.18092293976/9.18241306935/9.18278560175/s다. totals는 targets/log inserts 10000, SENT 7000, FAILED 1000, SKIPPED 2000, creation-stage PENDING 8000, log updates 8000, token updates 1010, fake attempts 9010, permanent failures 1010이며 provider fake failure rate 0.1120976692563818, final log failure rate 0.125다. 이는 조건부 before 근거이지 자동 이력서 성과가 아니다. creation token select 1,000→1회 최적화의 예상 prepared statements는 약 1,001이고 실제 after 개선 수치는 아직 미측정이다.
- TDD는 snapshot orchestration 파일 부재 `5/5 failures`, canonical fixture-before-capture ordering 보정, Redis TTL-intent schema mismatch RED를 각각 확인했다. 최소 GREEN 후 신규 snapshot contract `6/6`, 전체 issue-local Node contract `36/36`이 통과했다. actual Docker/DB/Redis/Gradle workload/restore는 0이며 baseline 수치나 개선 성과는 여전히 없다.
...
| 데이터 | DB 마이그레이션 수 | `src/main/resources/db/migration` | 12 (Flyway V1-V12, 2026-07-17 #194) | 추적 |
...
- #194 PostgreSQL 실행계획 기반 index before-baseline 시나리오 준비:
  - 상태: `scenario-ready / not-measured`. 이 세션에서는 Docker/PostgreSQL/`EXPLAIN`/`ANALYZE`를 실행하지 않았고 실제 baseline latency, buffer, scan, plan hash, 개선율을 생성하지 않았다.
  - 범위: #192/#193/#195/#196/#197/#198/#199의 핵심 read SQL 24개를 공통 1,000명 `datasetId + fixtureRunId`와 cross-issue report에 연결했다. production query source와 대조한 결과 reconstructed-current-query 2개, synthetic-candidate 22개, exact-current-production 0개이며 전부 `productionBeforeEligible=false`다. 모든 SQL은 `SELECT`/read-only CTE이고 기존 row 수정·삭제, Flyway/index/HypoPG/extension 변경은 0건이다.
  - 측정 계약: future runner는 `EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON)`을 read-only transaction에서 순차 실행한다. `WARM_RUNS`와 observer `ACTIVITY_SAMPLE_INTERVAL_MS`는 승인된 값을 runtime에 반드시 입력하며 기본값이 없다. actual Compose project canonical lock의 device/inode 소유권, lock 전후와 측정 종료 container/image/start/network identity, inspect `POSTGRES_DB`와 psql target의 실행 전후 server/database/postmaster identity, planner/analyze/autoanalyze/n-mod 및 PostgreSQL instance 전체 database의 client activity 무결성을 fail-closed로 검증한다. 현재 #193/#196/#199 artifact는 conditional/non-adoptable이므로 별도 사용자 승인 bridge 계약 전에는 cross-issue gate가 EXPLAIN 0회로 중단된다. 시작 오염은 EXPLAIN 전 중단하고 실행 중 오염 report는 `invalid/pending`과 non-zero 종료로 채택을 막는다. 첫 관찰을 true cold cache로 부르지 않고 `cold_like_observation`, 후속을 `warm_cache`로 분리하며 둘 다 `cacheResetPerformed=false`다.
  - 비교 계약: 재귀 parent-child topology를 포함한 정규화 plan structure SHA-256과 Seq/Index/Index Only/Bitmap scan, estimated/actual rows, loops, shared hit/read, rows removed, sort, planning/execution time을 저장한다. runtime 숫자만 달라진 동일 topology는 같은 plan hash, topology만 달라진 plan은 다른 hash가 되며 candidate predicate/order/join column은 제안 후보일 뿐 성과나 DDL 승인으로 해석하지 않는다.
  - TDD/검증: 최초 production 변경 전 Node 계약은 `5 tests / 5 failures` RED 뒤 `5 tests / 0 failures` GREEN이었다. 1차 PM finding 계약은 기존 5개를 보존한 채 `11 tests / 5 pass / 6 fail` RED 후 `11/11` GREEN이었다. 2차 PM finding은 기존 11개 GREEN을 보존하고 신규 8개가 RED였으며 artifact/anchor/source/schema/activity/rejected-report와 후속 경계 계약을 보강해 `23/23` GREEN이었다. 3차 PM finding은 cross-issue acceptance, instance-wide activity, Compose TOCTOU, sampling interval 신규 계약 `4 tests / 4 failures` RED를 test-only 커밋으로 남긴 뒤 전체 `27/27` GREEN으로 전환했다. 전체-diff 독립 리뷰에서는 NULL-state visibility, DB identity inspect bracket, long-interval SIGTERM의 신규 3개 계약이 `7 tests / 4 pass / 3 fail` RED였고 최소 수정 후 전체 `30/30` GREEN으로 전환했다. 후속 shutdown-bound 계약은 `8 tests / 7 pass / 1 fail` RED 뒤 느린 fake sample까지 포함해 전체 `31 tests / 31 pass` GREEN이다. 전체 관련 Node syntax, Bash syntax, JSON parse, 실제 source manifest 39개 hash, SQL read-only/meta-command safety, `git diff --check`가 성공했다. 이번 수정에서도 Docker/DB/psql/EXPLAIN/ANALYZE와 Gradle을 실행하지 않았다. 실제 성능 수치와 이력서 개선 문장은 exact production SQL capture, baseline 실행, 별도 index 승인 전까지 pending이다.
  - 최신 develop 보정: `origin/develop` `6796ed146244d8f3f5b5dd7048ebe16865084a97`의 #200/#201/#202/#206 API·Flyway·runtime drift를 신규 6개 계약 `6/6 RED`로 먼저 고정했고, 공통 무결성 감사 확장 시 10개 중 6개가 RED였다. #200 ownership/cardinality, #201 archive, #202 V11 RLS/JDBC role, #206 stable ordering, runtime-only Compose target/image, decimal-string PostgreSQL bigint와 `pg_stat_statements` continuity, 전체 50개 source hash, 최초 0600 rejection/`automaticAdoption=false`를 scenario-only로 보정해 기존 계약 포함 `41/41 GREEN`이다. 7개 cross-issue artifact는 모두 approval evidence schema가 없어 pending이며 actual Docker/DB/psql/EXPLAIN/k6와 성능 수치는 여전히 `not-measured`다.
  - #193 exact candidate handoff: finding-zero production branch `01faf929c24746e3a300e40d11171fc7d473954d`의 DB `summary/member-page/count-distinct` 3-query source hash/shape와 API 16-case 비교 matrix를 #194 scenario에 연결했다. 현재 develop 미통합·runtime SQL 미capture 상태이므로 `exact-production-branch-candidate-not-measured`, `productionBeforeEligible=false`, `automaticAdoption=false`다. V1의 `uk_charge_items_source`와 두 charge 후보의 prefix 중복·statement별 적용성·쓰기비용을 기록했고, `campus_members(campus_id,status,user_id)`는 기존 `uk_campus_members_campus_user` 대비 actual plan과 write cost를 증명할 때만 남는 conditional 후보다. leading-wildcard keyword index/extension은 승인 없이 후보로 채택하지 않았다. 실제 수치와 개선 성과는 여전히 0건이다.
  - 자체 리뷰 보정: 누적 counter 중 live/dead tuple 변화가 채택 gate에서 빠진 상태를 확장 계약 `10 tests / 9 pass / 1 fail` RED로 재현했다. 모든 PostgreSQL 누적 counter를 decimal-string에서 BigInt로 비교하고 변화별 machine-readable reason을 1개만 남기도록 수정한 뒤 current-develop 계약 `10/10`과 전체 issue-local 계약 `41/41`을 다시 통과했다.
  - 실제 plan/V12: PostgreSQL 17.10의 `charge_items` 749,017행, `campus_members` 50,603행에서 candidate index를 transaction-local 생성·rollback했다. #192 bulk source 조회는 `14.155ms/898 buffers -> 0.738ms/46`, #193 summary/page/count는 각각 `47.668/70.738/62.711ms -> 16.440/21.592/18.165ms`, #195 membership projection은 warm `6.199/4.625ms -> 1.115/0.921ms`였다. 이 근거로 V12에 18MB/12MB/1,568kB의 index 3개만 추가했고 #196/#197/#198/#199 및 wildcard/partial/include 후보는 제외했다. 임시 DB V1-V12 clean Flyway와 exact index 3개를 확인했으며 shared DB는 V11/index 0으로 유지했다. 이 수치는 SQL plan 근거이며 API 전체 개선률은 통합 After 3회 전까지 pending이다.
- #198 알림 생성·전송 before 시나리오 준비 (`scenario-ready / not-measured`):
  ...
  - `datasetId`와 canonical `fixtureRunId`를 분리하고, success/transient/permanent/inactive/no-token 분포를 모두 runtime 필수값으로 두었다. warmup/measured 11개 sample은 fixture를 반복 생성하지 않고 같은 prepared snapshot을 복원하며, 승인되지 않은 batch size·outcome 비율은 정하지 않았다.
  - Firebase credential 존재, fake가 아닌 sender, `test|docker|prod` profile, 사용자 승인 project와 다른 Compose label, manifest의 Compose project/database 불일치, 실제 endpoint 불일치, dirty worktree, `faithlog-latest` shared stack을 즉시 거부한다. Spring context-startup coffee seed runner도 mock으로 대체한다. #198 global lock과 canonical Compose-project lock을 함께 사용하고, 같은 project lock 선점 시 fixture SQL/Gradle을 0건으로 유지한다. fixture는 `PERFORMANCE_198_DUMMY:` token과 fixture marker log만 허용하고 non-fixture token mutation, dedupe replay, upstream campus/user target equality, cross-user failure continuation과 mixed-token 사용자 내부 partial failure를 correctness 계약으로 검증한다.
  - 수집 준비 지표는 생성/전송/end-to-end duration·throughput p50/p95/p99/max, Hibernate prepared statements와 per-user DB calls, harness-lifecycle PostgreSQL/Redis counter delta, Java/container CPU·memory, log/token insert/update count, provider fake failure rate와 final log failure rate다. initial/before/after/final container·server identity를 고정하고 PostgreSQL exact schema/stats reset/captured ordering, Redis run ID/uptime/DBSIZE/SET, approved cadence의 PostgreSQL+Redis Docker sample 최소 2시점을 엄격 검증한다. missing/null/string/mixed-container/one-sample evidence와 logical write 대비 under/over-count를 모두 거부한다.
  - additive fixture 상태는 승인된 `snapshot-restore`로 격리한다. canonical fixture 1회 뒤 snapshot 1회, warmup 1회와 measured 10회 각각 직전 restore 11회를 수행하고 PostgreSQL/Redis state·cardinality·TTL-intent receipt가 모두 일치할 때만 다음 sample로 진행한다. summary는 계속 `accepted=false`, `automaticAdoption=false`이며 실제 fixture/job/Docker/성능 DB/baseline은 실행하지 않았으므로 수치와 개선 성과는 `not measured`다.
  - TDD는 최초 구현 파일 부재로 Node 계약 `5 tests / 5 failures` RED를 확인·커밋했다. PM 전체 diff finding 5건도 별도 계약 `5 tests / 5 failures`로 재현·커밋한 뒤 합성 non-Docker evidence와 fake lock/runtime replacement harness를 포함한 전체 Node 계약 `11 tests / 0 failures` GREEN으로 전환했다. 기존 Java harness compile, Bash/Node syntax, 전체 `450 tests / 0 failures / 4 skipped`, `./gradlew build`, `./gradlew asciidoctor` 이력은 유지하며 신규 guarded harness 1건은 기본 실행에서 정상 skip됐다.
...
- #196 기도조·투표 목록 before 시나리오 준비:
  ...
  - PM review finding을 반영해 실제 Compose project canonical lock, endpoint별 warmup/measured 분리, JWT 잔여 수명 gate, immutable executionRunId report, exact/monotonic DB counter와 planner/analyze/external-activity gate, 두 k6 summary shape 및 credential child 격리를 계약 테스트로 고정했다. 상태는 계속 `scenario-ready / not-measured`이며 이 항목 자체를 성능 성과로 집계하지 않는다.
  - PM 2·3차 review finding을 반영해 exact planner/table/maintenance schema, vacuum/autovacuum drift, runtime 승인형 sampling coverage, app/DB/PostgreSQL continuity, runtime-required target identity, decimal-string/BigInt DB counter를 고정했다. Sampling/adoption 정책은 미승인이므로 clean evidence도 `conditional-not-adoptable`이며, 합성 evidence 외 실제 baseline 수치나 개선 성과는 없다.
  - PM 후속 review에서 seed/shape/direct k6까지 target default 제거, 1초/2초 상수 없는 runtime sampling 결속, 당시 runtime의 exact resource set 및 finite/nonnegative resource gate를 RED→GREEN 계약으로 보강했다. 이후 current-develop 감사에서 Redis를 포함한 exact 3-container/full-ID 계약으로 갱신했다. 이는 scenario 준비 검증이며 실제 측정 또는 성능 개선 수치가 아니다.
  - PM 5차 review에서 lock 대기 중 same-name runtime 교체, BASE_URL address-family 불일치, malformed/impossible RAM evidence를 `16 tests / 11 pass / 5 fail` RED로 재현했다. seed/shape/run이 lock 전 immutable app/DB/Compose/published endpoint/PostgreSQL snapshot과 lock 후 identity를 exact 대조하고, numeric loopback binding exact 1개와 strict memory byte/`0..100%` gate를 공유하도록 보강했다. 최종 fake/static 계약은 `16 tests / 0 failures`이며 실제 baseline은 여전히 측정하지 않았다.
  - 2026-07-16 최신 develop source/Flyway/API drift와 공통 무결성을 다시 감사했다. dataset v2는 generic Poll 비페이지 배열과 ID 내림차순, MEAL `PageResponse`의 explicit `id,desc`/90일 archive, #200 CUSTOM·COFFEE·MEAL `manageableByMe`, #202 27-table RLS/JDBC owner 무영향을 고정한다. app/DB/Redis full-ID·image·StartedAt·Compose/DB/Redis process continuity, pgss available/unavailable 안정성, k6 v2 두 metric shape, decimal-string/BigInt counter, exact 3-container CPU/RAM/cadence, immutable namespace와 최초 rejection 보존을 fake/static evidence로만 검증한다.
  - 여러 성능 이슈의 test-code 보정은 병렬 가능하지만 shared stack 실제 부하는 PM exclusive window에서 순차 실행한다. #196은 `automaticAdoption=false`, `conditional-not-adoptable`, `scenario-ready / not-measured`이며 이번 보정에서도 Docker/DB/HTTP/seed/k6 실행과 baseline/개선 수치 생성은 0건이다.
  - actual handoff read-only 감사에서 clean detached `6796ed146...` checkout과 이후 생성 app image의 PM 승인 operational provenance, app/DB/Redis full identity, Flyway 11·27/27 RLS·pgss unavailable, fresh `i196-20260716-a` namespace를 확인했다. OCI revision/digest 부재는 limitation이고 app의 SQL DEBUG/format env 미확인은 slot 전 runtime 준비 항목이다.
  - conditional clean report가 첫 endpoint에서 runner를 중단하던 handoff gap을 `22 tests / 21 skipped / 1 fail` RED로 고정하고, conditional은 27 endpoint 순차 수집을 계속하되 전체 종료는 exit 2, rejected는 first-failure stop으로 최소 보정했다. 최종 fake/static 계약은 `22 tests / 0 failures`이며 actual HTTP/seed/shape/k6와 성능 수치는 여전히 0이다.
  - PM handoff 재검증에서 실행형 fake orchestration을 추가해 single prayer mode의 5 endpoint conditional 순차 진행·최종 exit 2, rejected/malformed/missing report의 첫 endpoint 중단, k6/resource sampler/integrity/runtime failure의 후속 endpoint 0을 검증했다. 첫 focused RED는 `23 tests / 22 skipped / 1 fail`로 operational failure에도 먼저 continuing을 출력하던 문제를, 다음 전체 RED는 `23 tests / 22 pass / 1 fail`과 `status=null`로 60초 fake timeout을 성공처럼 볼 수 있는 반례를 고정했다. runner는 operational status가 모두 0인 뒤에만 conditional continuation을 기록하며, optional `PERF_REPORT_ROOT` 아래 immutable fixture/execution 경로로 artifact를 격리한다. Fake subcase는 error/signal 부재와 exact exit status 및 15초 미만을 강제하고 focused 33.9초, 전체 `23 tests / 0 failures` 64.3초로 GREEN이다. 상태는 계속 `scenario-ready / not-measured`다.
  - actual before instrumentation handoff는 app-only evidence override, statement-only SQL filter, bind/extract logger OFF, 새 app full identity, unrelated app-env drift를 막는 secret-free before/after digest, clean exact scenario HEAD와 tracked tooling aggregate SHA-256, immutable prep attempt/partial-rejected receipt를 계약화했다. 실제 Docker lifecycle/fixture/HTTP/k6 실행 및 baseline 수치 생성은 PM exclusive slot 전까지 0건이며 상태는 계속 `scenario-ready / not-measured`다.
  - 이후 PM exclusive actual의 `g02`는 2 endpoint report 약 1.6 GiB와 `prayer_groups` SQL artifact 약 1.3 GiB를 남긴 뒤 rejected로 중단돼 valid baseline이 아니다. `g01`/`g02` 및 이전 evidence/ID는 삭제·수정·재사용하지 않는다. 대형 SQL과 daemon log 팽창 반례는 test-only `7 tests / 2 pass / 5 fail` RED로 고정했고, measured 전 live Docker follower·first/final sentinel·statement-only create-only gzip, digest와 uncompressed byte/line/statement count, streaming gunzip validator/summarizer, app `local` log-driver rotation identity, endpoint별 cap과 최소 2 GiB headroom projection을 최소 도구 변경으로 보강했다. Installed Docker가 의도적 follower 중단을 exit `143`으로 반환하는 추가 반례는 focused `1 fail` RED 뒤 exact self-stop에서만 허용해 GREEN으로 전환했고, 최종 issue-local 계약은 `52/52` GREEN이다. 마지막 소형 smoke는 sentinel 2개와 statement 1개를 `57` compressed bytes/`37` uncompressed bytes, line/statement count `1/1`로 검증했으며 HTTP/DB/seed/shape/k6/lifecycle은 0건이다. 보존 원문을 복사·삭제하지 않은 read-only gzip 계산은 두 SQL 파일을 3,477,768/12,842,803 bytes로 산출했다. 초기 보수적 64 MiB SQL cap projection은 당시 디스크를 차단했고, PM은 observed maxima 근거로 fresh actual의 SQL gzip 32 MiB/endpoint, non-SQL 8 MiB/endpoint, app local log 64 MiB×3, safety 2 GiB를 명시 승인했다. Exact 27 endpoint 필요량은 3,481,272,320 bytes이고 최종 확인 가용 4,108,783,616 bytes보다 627,511,296 bytes 작다. Partial 수치나 latency/query 수치는 성과로 채택하지 않으며 상태는 `scenario-ready / not-measured`다.
  - Fresh `h02`는 prep/fixture/shape 및 첫 Prayer endpoint를 통과한 뒤 `prayer_groups` resource boundary false rejection으로 중단됐다. Measured end 뒤 nearest complete sample은 413 ms였지만 SQL 후처리 동안 계속된 마지막 sample이 3.915초 뒤라 old last-row validator가 승인 max gap 3초를 잘못 적용했다. Test-only RED에서 extra valid post-end samples 반례를 고정하고, latest pre-start/earliest post-end 경계 선택과 선택 구간 minimum count를 적용하되 전체 identity/strict monotonic/consecutive max-gap 검증은 유지했다. 보존된 `h02` evidence read-only 재검증은 `conditional-not-adoptable` 및 policy reason 하나만 남겼다. 원 `h02`는 rejected/non-reusable이며 수치 성과로 집계하지 않는다. `h03` prep ID는 stale tooling-HEAD preflight에서 lifecycle/fixture/load 0건으로 소비돼 재사용하지 않는다.
  - Fresh `h04`는 prep/fixture/shape와 첫 두 measured Prayer endpoint를 HTTP/custom failure 0으로 완료했다. `prayer_current_season` 88,169 requests/p50 5.41 ms/p95 12.14 ms/p99 24.30 ms/max 308.47 ms/734.635 requests/s, `prayer_groups` 2,041 requests/p50 256.50 ms/p95 526.80 ms/p99 778.71 ms/max 1.48 s/16.953 requests/s는 모두 rejected partial diagnostic-only이며 baseline/성과가 아니다. 세 번째 `prayer_assignable` warmup 245건은 정상 API 응답에도 두 campus 공통 creator 1명을 isolation 오염으로 오판해 measured 0으로 중단됐다. 격리 deny-set을 isolation-only 49명으로 보정하고 assignable/weekly-board가 공통 사용하게 했으며 exact order/count와 malformed/duplicate ID fail-closed를 유지했다. Test-only RED, installed k6 inspect, issue-local `53/53` GREEN을 확인했다. `h04`는 immutable/non-reusable이고 다음은 fresh `h05`다.
  - SQL follower가 parent env를 상속하는 보안 반례는 capture Node/Docker와 runner sentinel Docker 각각 focused `1 fail` RED로 고정했다. GREEN은 shell/Node/Docker 모든 경계에서 empty-env allowlist만 사용하며 credential/token/DB password/caller unknown env를 child에 전달하지 않는다.
  - 전체 병렬 계약 재검증에서 measured k6가 짧을 때 resource sampler startup보다 stop marker가 먼저 생성될 수 있는 race를 `51/52`로 재현했다. Sampler create-only ready marker와 runner bounded wait를 추가해 measured 시작 전 sampler 준비를 강제했고, focused `3/3` 및 전체 `52/52` GREEN으로 다시 고정했다.
  - Full-suite 병렬 재현에서 resource sampler가 reader 초기화 전에 measured k6와 race하는 `51/52` RED를 확인했다. Sampler create-only 0600 ready marker와 runner의 child-aware bounded wait를 추가하고 delayed-start fake에서 `sampler-ready`가 measured `k6-start`보다 먼저임을 실행형으로 고정했다.
  - production 코드, API, 권한, DB/Flyway, dependency는 변경하지 않았다. 이번 storage 보정에서는 actual 재시도·DB·HTTP·seed·shape·k6·lifecycle을 실행하지 않았고, installed Docker 검증은 기존 app log에 sentinel 2개와 비민감 statement 1줄을 남긴 소형 follower smoke로만 제한했다. 실제 성능 숫자와 개선 문장은 승인된 유효 전후 비교가 끝난 뒤에만 기록한다.
- #188/#189/#190 통합 검증:
...
- 2026-07-16 #197 current-develop 성능 기준선 준비
  ...
  - actual-before 준비 blocker였던 fresh fixture 생성 부재와 사전 생성 불가능한 DB-wide exact activity signature 요구를 RED로 고정했다. namespace 0행 확인 후 create-only API 한 경로로 warmup 1명·measured 1,000명·rollback 1명, success/rollback campus, PENALTY 계좌/규칙/멤버십을 준비하고 마지막 login으로 1,002개 runtime-only JWT manifest를 만드는 prepare/inspect 계층을 추가했다.
  - exact HTTP/custom Counter와 fixture-owned direct SQL cardinality(weekly 1,000 user/daily 7,000 row/charge 1,000 source-user binding·금액 합계/rollback persisted 0)는 유지하고, DB-wide/table/pgss 누적치는 BigInt monotonic·외부 session 0·planner/maintenance/runtime continuity의 source-unattributed supporting evidence로 분리했다. shared-stack 결과는 clean이어도 `conditional-not-adoptable`, `automaticAdoption=false`이며 calibration, tolerance, sleep, subtraction, background estimate는 사용하지 않는다.
  - prepare fake full-run은 초기 exact 3,014 API 호출에서 시작했고, fresh I rollback 조사 뒤 두 campus 동일 rule 8개를 포함한 exact 3,018 API 호출로 보정했다. mode-600 secret input/credentials, token/password 비노출, partial failure receipt와 no-cleanup/no-reuse를 계속 검증한다. installed k6 v2 no-HTTP synthetic은 Counter 1과 Rate `{rate:0,passes:0,fails:1}`을 재현했다.
  - source provenance의 구 Java package path가 migration-only digest를 허용하던 gap을 RED로 재현했다. current devotion/notification/batch scheduler/batch service/migration 다섯 revision tree의 개별 존재를 강제하고 154-entry corrected digest `625bc9e...c2b2a9d`로 handoff를 보정했으며, actual load나 성능 수치로 집계하지 않았다.
  - fresh C preflight는 fixture HTTP/k6 0 상태에서 PostgreSQL evidence `127.0.0.1/32`와 승인 host `127.0.0.1`의 표기 차이로 중단됐다. actual evidence 원문을 보존하면서 IPv4 `/32`·IPv6 `/128` host CIDR만 같은 explicit loopback으로 비교하고 외부/다른 IP·prefix를 거부하도록 보정했으며 C는 재사용하지 않고 fresh D로 이관한다.
  - fresh D는 runtime/namespace/API seed/DB preflight와 HTTP 3,014건, campus 2/account 1/rule 4/user·membership·token 각 1,002, penalty 2,250 준비를 마쳤지만 마지막 installed k6 v2 inspect에서 OS env assignment가 `__ENV`로 전달되지 않아 exit 107로 중단됐다. measured load는 0이고 D fixture/evidence는 보존·재사용 금지다. prepare inspect와 baseline phase 모두 여섯 init 입력을 명시적 `-e`로 전달하도록 실행형 RED→GREEN 보정했으며 raw credential/token은 argv/output에 두지 않는다. installed inspect/no-HTTP synthetic을 포함한 issue-local Node 계약은 75/75다. 다음 candidate는 fresh E이고 상태는 계속 `scenario-ready / not-measured`다.
  - fresh E는 fixture prepare와 measured 1,000건, HTTP/check/custom failure 0을 완료했지만 serial blocking Docker stats의 same-role gap이 승인 2초를 넘어 stage `measured`에서 rejected됐다. p50 42.58ms/p95 101.28ms/p99 154.74ms/max 196.32ms/throughput 592.299163/s는 diagnostic-only이며 baseline·성과로 채택하지 않는다. exact 세 full-ID shared-timestamp snapshot, continuous stream, post-marker final boundary, max-gap deadline과 capture failure propagation으로 sampler를 보정했고 issue-local Node 계약 77/77을 통과했다. E는 read-only 보존·재사용 금지이고 다음 candidate는 fresh F다.
  - fresh F는 fixture prepare와 measured 1,000건/failure 0을 완료했지만 Docker streaming의 exact `ESC[H`/`ESC[K` screen framing 때문에 full-ID parser가 sampler를 measured 종료 전 거부했다. p50 79.56ms/p95 222.09ms/p99 364.84ms/max 537.42ms/throughput 300.159325/s는 diagnostic-only이며 baseline·성과로 채택하지 않는다. `.ID` format과 control-free 또는 exact whole-snapshot framing만 허용하고 unknown/mid-field/malformed control을 거부하도록 보정했다. installed current three-role Docker read-only snapshot도 exact full ID/shared timestamp로 통과했고 issue-local Node 계약은 78/78이다. F는 보존·재사용 금지, 다음 candidate는 fresh G다.
  - fresh G는 fixture prepare와 measured 1,000건/failure 0을 완료했지만 snapshot 사이 standalone `ESC[K`와 recurring `ESC[JESC[H`를 data row로 오인해 sampler가 final boundary 전에 거부했다. p50 43.42ms/p95 102.72ms/p99 160.66ms/max 249.91ms/throughput 596.255989/s는 diagnostic-only이며 baseline·성과로 채택하지 않는다. exact initial/separator/recurring/data-suffix state machine과 malformed-control 반례를 고정했고, installed Docker read-only 3-snapshot smoke는 `stream-samples`에서 exact full ID 9행, stop marker/final boundary, 0.497/0.501초 cadence를 통과했다. issue-local Node 계약은 79/79다. G는 보존·재사용 금지, 다음 candidate는 fresh H다.
  - fresh H는 prepare/warmup 1/measured 1,000과 failure 0을 완료했고 direct join은 weekly user 1,000, daily 7,000/1,000 users, source-user 결속 charge 1,000/각 2,250원/합계 2,250,000원, campus warmup 포함 1,001을 확인했다. 누적 통계 publication은 target 595/4,165/595와 DB insert 5,355만 보였고 users update 9/다른 DB read도 함께 증가해 기존 exact-zero gate가 measured에서 거부했으며 rollback은 미도달했다. p50 46.86ms/p95 103.30ms/p99 130.02ms/max 189.19ms/throughput 549.373879/s는 diagnostic-only다. direct cardinality를 rollback 전 exact gate로 옮기고 누적 delta는 source-unattributed supporting-only로 기록하도록 보정했다. H는 영구 보존·재사용 금지, 다음 candidate는 fresh I다.
  - fresh I는 prepare, warmup 1, measured 1,000, measured direct cardinality와 runtime continuity를 통과했지만 rollback request가 HTTP success로 응답해 expected error check 0/3, failure rate 100%, exit 99로 rejected됐다. p50 50.14ms/p95 110.04ms/p99 145.10ms/max 187.10ms/throughput 525.637993/s는 diagnostic-only다. current source는 활성 rule 계산 합계가 0이면 계좌 조회를 생략하고, I prepare는 rollback campus에 rule을 만들지 않았으므로 서버가 아니라 stale fixture expectation이 원인이었다. J부터 두 campus에 같은 rule 4개씩을 만들고 rollback campus만 계좌 0개로 유지해 positive 벌금 계산 뒤 missing-account 예외로 weekly/daily/charge persisted 0을 증명한다. I는 영구 보존·재사용 금지이며 next candidate는 fresh J다.
  - fresh J는 warmup 1, measured 1,000 failure 0, rollback `400 BILLING_REQUIRED_PAYMENT_ACCOUNT_MISSING`과 weekly/daily/charge persisted 0을 통과한 유효한 shared-stack conditional before다. measured p50 65.0935ms/p95 145.86005ms/p99 180.06974ms/max 272.145ms/throughput 400.1357260382722/s, direct weekly 1,000/daily 7,000/charge 1,000/duplicate 0/금액 합계 2,250,000원, app max CPU/RAM 314.19%/715,548,262 bytes, DB 163.31%/288,568,115 bytes, Redis 6.77%/21,820,867 bytes를 기록했다. `conditional-not-adoptable`, `automaticAdoption=false`이며 cumulative DB/table/pgss publication은 supporting-only다. J evidence는 영구 read-only/non-reusable다.
  - current Java call graph에서 weekly submit 한 건이 동일 daily 7행을 날짜별 7회 조회하고 summary에서 다시 조회하는 구조를 확인했다. focused RED 후 daily 7행 1회 load + `recordDate` Map upsert + 누락 행 `saveAll` 1회로 보정해 static repository read 계약을 8회에서 1회로 줄였다. 이는 예상 호출 감소이며 실제 after latency/CPU/throughput 개선 수치는 아직 측정하지 않았다. API/권한/transaction/rollback/벌금/source/snapshot은 유지하고 frontend/Flyway/index/dependency는 변경하지 않았다.
  - 상태는 `before-recorded / after-not-measured`. 수정 서버를 별도 integration runtime에 올린 뒤 PM 단일 load로 after를 측정하기 전에는 개선 성과를 주장하지 않는다.
...
- Focused announcement/media tests and REST Docs are GREEN. Independent full-diff review added RED→GREEN coverage for durable variant cleanup, READY temporary-original cleanup and finalize idempotency, exact 10-minute URL TTL, Redis-unavailable 503 behavior, strict ACTIVE-member reads, rollback-safe required notification deduplication, ordered 100-ID DB validation batches, future-only schedule validation, inactive-category retention, and flushed duplicate conflicts.
- Final `./gradlew clean test build asciidoctor` is rerun after every review correction; the final XML/build values are recorded after the last source correction below.
- Final exact-HEAD post-review gate completed successfully in 3m43s. Gradle XML contains 146 suites / 740 tests / failures 0 / errors 0 / skipped 9; bootJar, jar, REST Docs HTML, and JaCoCo report were generated.
- The first final-gate attempts stopped before test completion because the host filesystem had only 116–128MiB free. Only regenerable Gradle 9.3.1 and CocoaPods download caches were removed; source, simulator artifacts, Docker state, and user data were not changed.
...
- PM review RED→GREEN additionally covered the V14 notification type CHECK, expired PENDING finalize rejection before external I/O, ordered public cache SHA-256 metadata, typed presign failure handling, and PostgreSQL same-campus attachment foreign keys.
- A later ownership regression test showed that the full replacement validator incorrectly reapplied uploader ownership to images already attached to the announcement. The minimal fix distinguishes existing attachment IDs from newly introduced IDs: authorized co-managers can retain or reorder existing READY images, while newly added assets still require requester ownership; removal remains ORPHANED and update creates no new outbox event.
- The exact-source clean gate completed in 7m14s with 147 suites / 747 tests / failures 0 / errors 0 / skipped 10, plus build and Asciidoctor success. After correcting the PostgreSQL fixture to satisfy `request_id`, user, and campus requirements with prepared statements, the dedicated ephemeral PostgreSQL 17 gate completed 4/4 with no failures or skips. It verified clean V1→V14, V13→V14 upgrade, the exact notification type CHECK boundary, cross-campus attachment FK rejection, and the unchanged V13 duplicate-email failure contract without touching the shared QA database.
- After the co-manager attachment correction, `./gradlew clean test build asciidoctor` completed in 3m59s with 147 suites / 753 tests / failures 0 / errors 0 / skipped 10. The PostgreSQL gate was not rerun because Docker Desktop's API socket stopped responding; the daemon was not restarted to protect shared work. The previous stable V14 actual gate remains 4/4 GREEN, and this correction changes no migration or persistence mapping.

## 2026-08-03 Issue #238 Poll notice, images, and publication notifications

- Reused the reviewed Issue #237 media foundation and integrated its final shared-editor stable commit `44ce9c600ca3862420389ed961594aa041a23f75` through Git ancestry; no storage signing, image processing, cleanup, or rate-limit implementation was duplicated.
- Added 5,000-character optional poll notice, authorized title/notice/image-set updates, ordered poll attachments, list payload hints, and detail image IDs across all five poll types while preserving existing poll option, response, close, and settlement contracts.
- Added a poll-row-serialized durable publication outbox, ACTIVE-member targeting with creator exclusion, all-type notification mapping, ID-only payload, scheduled OPEN processing, retryable downstream failure, and poll-level idempotency.
- V16 owns poll notice, tenant-safe `poll_images`, outbox, scheduled index, and notification CHECK changes. Static Flyway and protected PostgreSQL fixture contracts are included; no shared QA database, Docker, R2, or FCM operation was run in development.
- Focused notice/media/outbox/scheduler/REST Docs, retention, yearly recap, architecture, and existing Poll service regression gates are GREEN. After both prerequisite histories were integrated, the final clean `test build asciidoctor` gate passed 810 tests / failures 0 / errors 0 / skipped 12 in 4m20s and generated 161 XML suites plus REST Docs HTML. No performance or delivery-rate claim is made.
- Integration order is preserved in both Git ancestry and physical migration files: Issue #237 stable V14, reviewed Issue #236 `4c5b13b3276d2c1bff7418d8983ce32cfb1333c5` V15, then Issue #238 V16. A test-first RED caught the stale V14-to-V16 fixture; GREEN now requires clean V16 and starts the V16 upgrade at V15.
- PM review reproduced the V16 retention FK rollback and cross-domain infrastructure coupling as test-only REDs. The GREEN path now orphans expired poll media and deletes image/outbox children in explicit FK order, with an independent-transaction rollback test that clears the persistence context before re-reading preserved state.
- The approved joint-editor rule distinguishes existing attachment IDs from newly introduced IDs: authorized managers/duties can retain or reorder another uploader's existing poll images, while a new unattached asset owned by another user remains `MEDIA_ASSET_INVALID`. Removal still transitions only the removed asset to ORPHANED and no edit creates another publication outbox.
- Poll, announcement, and media dependencies now point through four narrow consumer-owned ports and relative-domain adapters. Architecture source tests reject direct foreign-infrastructure imports; focused attachment conflict, published/visible read, manager preview, duty upload, retention, and settled-MEAL regressions are GREEN.
- A dedicated disposable PostgreSQL 17 container verified clean V1-to-V18 and protected V13/V15/V16/V17 upgrade paths, including V16 notification/FK boundaries, V17 compact archive schema, and V18 retry/lease constraints. An overlong test-only campus invite code was corrected within its existing 50-character schema; no production constraint was relaxed and the shared QA database was untouched.
- Post-integration review added portrait PNG/JPEG coverage and corrected thumbnail/detail resizing to bound the longest dimension at 480/1600 without enlargement. Focused processor tests are GREEN.
- User-approved follow-up GREEN removes poll participation from the recap, adds own non-deleted comment count and exact own devotion-penalty aggregates, and uses V17 compact facts plus coverage watermarks so retention cannot silently shrink a complete recap. Archive upsert and source deletion share one transaction; a fresh-transaction regression proves archive failure preserves previously selected source rows.
- Full-diff review caught an extra historical-data boundary: live and archive queries incorrectly required current ACTIVE campus membership for the user's own comments and penalties. A focused integration RED reproduced both zeroed aggregates; GREEN now keeps ACTIVE filtering only for campus journey and preserves own historical facts without exposing other users or campuses.
- V18 adds durable cleanup attempt/backoff/lease metadata. Executable service and repository tests prove 100 failed head candidates do not starve candidate 101, transient failures retry, backoff caps at 24 hours, and an expired multi-instance claim is recoverable. A later hard-crash regression also recovers only PROCESSING rows stale for 24 hours, deletes all tracked temporary/variant keys through the same generic retry path, and leaves active PROCESSING untouched.
- Two actual Spring/JPA thread orderings fix the finalize/cleanup race contract: finalize-first commits READY and cleanup removes only the expired temporary original, while cleanup-first commits FAILED+lease and a concurrent finalize is rejected before the cleanup-owned keys and row are removed. These are correctness and storage-retention improvements, not measured latency or cost-savings claims.
- Final whole-diff review found that the annual archive writer could pass a full 1,000-member year's source IDs through one `IN` predicate. A test-first correction bounds lookup and write batches to 500 while preserving idempotency and the archive/delete transaction. The final exact-HEAD clean gate is 843 tests / failures 0 / errors 0 / skipped 15 with build and Asciidoctor success; a disposable PostgreSQL 17 gate passed all 9 clean/upgrade migration cases without touching the shared QA database.
- A second whole-diff check found that comment recap attribution followed the poll start year instead of the approved comment-written year. A cross-year integration RED now fixes both live aggregation and V17 archive facts to the comment's own `created_at` in `Asia/Seoul`.
- V17's initial coverage watermark now derives its year from `CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Seoul'` instead of session-timezone `CURRENT_DATE`. The disposable PostgreSQL 17 gate passed 9/9 clean and protected upgrade cases, including identical Seoul year boundaries under UTC and America/Los_Angeles sessions and the V18 stale-PROCESSING partial index.

## 2026-08-04 Issue #239 커피 투표 자동 마감 장애 격리
...
- 최종 retention 변경 뒤 weeklymaterial/announcement/media/Flyway contract 회귀는 173 tests / failures 0 / errors 0 / skipped 0으로 통과했고 `build asciidoctor -x test -x jacocoTestReport`도 성공했다. 전체 `test build asciidoctor` 재시도는 관련 없는 `PasswordResetTransactionIntegrationTest` Spring context 생성 중 `OutOfMemoryError`가 발생해 중단했다.
