...
- Fresh provisioning TDD: #192 dataset의 `999 USER + 1 ADMIN` 구성은 #195의 exact `1,000 ACTIVE USER`, admin-excluded 계약에 맞지 않아 재사용을 금지했다. Fresh namespace/report reservation, public signup-only 1,000명, collision/partial/role/active/duplicate/credential/3-container continuity 계약을 test-only RED로 고정한 뒤 issue-local provisioner를 구현했다. 추가 PM finding에서 30분 access token을 2,000여 요청에 재사용하는 경계를 `9 pass / 7 fail` RED로 재현하고, signup 직후 fresh login + runtime `exp` margin + verification 요청별 조건부 refresh로 보정했다. Targeted `16/16`, 전체 issue-local `59/59`가 GREEN이다. 실제 provisioning/HTTP/DB/Docker/k6는 실행하지 않았고 상태는 계속 `scenario-ready/not-measured`다.
