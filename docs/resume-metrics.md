...
- #194 PostgreSQL 실행계획 기반 index before-baseline 시나리오 준비:
  ...
  - 측정 계약: future runner는 `EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON)`을 read-only transaction에서 순차 실행한다. `WARM_RUNS`는 승인된 값을 runtime에 반드시 입력하며 기본값이 없다. actual Compose project canonical lock의 device/inode 소유권, inspect `POSTGRES_DB`와 psql target의 실행 전후 server/database/postmaster identity, planner/analyze/autoanalyze/n-mod/external activity 무결성을 fail-closed로 검증한다. 시작 오염은 EXPLAIN 전 중단하고 실행 중 오염 report는 `invalid/pending`과 non-zero 종료로 채택을 막는다. 첫 관찰을 true cold cache로 부르지 않고 `cold_like_observation`, 후속을 `warm_cache`로 분리하며 둘 다 `cacheResetPerformed=false`다.
