...
| 유지보수 | 주요 모듈 수 | 패키지/도메인 기준 | 10 top-level modules, 590 Java sources including tests (2026-07-12 #176) | 추적 |
...
- #176 Refresh Token Rotation 원자성 및 동시 재사용 차단:
  ...
  - 원자 전이: `RefreshTokenStore.rotate`가 `ROTATED/REJECTED` 결과를 반환하고 production Redis adapter는 revoked marker 부재 확인, expected JTI 비교, new JTI와 14일 TTL 교체를 하나의 Lua script에서 수행한다. test adapter도 synchronized CAS로 같은 전이를 재현한다. candidate access/refresh는 CAS 전에 생성하되 승자만 응답한다.
