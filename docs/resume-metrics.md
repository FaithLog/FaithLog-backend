...
- 2026-07-16 #197 current-develop 성능 기준선 준비
  ...
  - exact HTTP/custom Counter와 measured business row(weekly 1,000/daily 7,000/charge 1,000, rollback persisted 0)는 유지하고, DB-wide/table/pgss 누적치는 BigInt monotonic·외부 session 0·unexpected table write 0·planner/maintenance/runtime continuity의 supporting evidence로 분리했다. shared-stack 결과는 clean이어도 `conditional-not-adoptable`, `automaticAdoption=false`이며 calibration, tolerance, subtraction, warmup 비례 추정은 사용하지 않는다.
