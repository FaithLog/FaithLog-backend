...
| 2026-07-14 | #200 full regression/build/docs | 성공 | 최종 `./gradlew test` 465 tests / 0 failures / 0 errors / 3 skipped, `./gradlew build` 성공, `./gradlew asciidoctor` 성공, REST Docs snippet group 153개, `git diff --check` 성공 | PM 전체 diff 리뷰 요청. Docker/PostgreSQL 실마이그레이션은 사용자 금지에 따라 미실행 |
