...
| 영역 | 지표 | 측정 방법 | 최신값 | 목표 |
| --- | --- | --- | --- | --- |
| 품질 | 테스트 통과율 | `./gradlew test` | 100% of executed tests (2026-07-13 #189, 423 tests / 0 failures / 0 errors / 3 skipped) | 100% |
...
| 품질 | 인증/문서 스니펫 묶음 수 | `find build/generated-snippets -mindepth 1 -maxdepth 1 -type d` | 141 snippet groups (2026-07-13 #189) | 증가 추적 |
...
- #189 밥 담당과 투표 항목 그룹별 후청구:
  ...
  - 전체 검증: Duty/Poll/Billing/Flyway/REST Docs focused GREEN, 전체 `423 tests / 0 failures / 0 errors / 3 skipped`, `./gradlew build`, `./gradlew asciidoctor`, `git diff --check`가 성공했다. test source는 80개, Java source/test는 634개, REST Docs snippet group은 141개이며 MEAL 관련 group 17개와 렌더된 `index.html`을 확인했다.
  ...
  - 이력서 문장 후보: `다수 MEAL 담당자·본인 계좌·즉시 OPEN 투표와 option group별 후청구를 정규화하고 exact ceiling 산술·pessimistic lock·DB unique·단일 transaction으로 1회성 정산을 보장했으며, 423개 전체 테스트와 141개 REST Docs 계약으로 권한·격리·rollback을 검증했다.`
