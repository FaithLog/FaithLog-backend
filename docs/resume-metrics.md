...
- #194 PostgreSQL 실행계획 기반 index before-baseline 시나리오 준비:
  ...
  - TDD/검증: 최초 production 변경 전 Node 계약은 `5 tests / 5 failures` RED 뒤 `5 tests / 0 failures` GREEN이었다. 1차 PM finding 계약은 기존 5개를 보존한 채 `11 tests / 5 pass / 6 fail` RED 후 `11/11` GREEN이었다. 2차 PM finding은 기존 11개 GREEN을 보존하고 신규 8개가 RED였으며 artifact/anchor/source/schema/activity/rejected-report와 후속 경계 계약을 보강해 `23/23` GREEN이었다. 3차 PM finding은 cross-issue acceptance, instance-wide activity, Compose TOCTOU, sampling interval 신규 계약 `4 tests / 4 failures` RED를 test-only 커밋으로 남긴 뒤 전체 `27 tests / 27 pass` GREEN으로 전환했다. 전체 관련 Node syntax, Bash syntax, JSON parse, 실제 source manifest 39개 hash, SQL read-only/meta-command safety, `git diff --check`가 성공했다. 이번 3차 수정에서도 Docker/DB/psql/EXPLAIN/ANALYZE와 Gradle을 실행하지 않았다. 실제 성능 수치와 이력서 개선 문장은 exact production SQL capture, baseline 실행, 별도 index 승인 전까지 pending이다.
