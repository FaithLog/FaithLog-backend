...
- #196 기도조·투표 목록 before 시나리오 준비:
  ...
  - PM 2차 review finding을 반영해 exact planner/table/maintenance schema, vacuum/autovacuum drift, 1초 sample contract·2초 max gap·boundary/minimum coverage, app/DB/PostgreSQL runtime continuity와 operator exclusive-window 선언을 추가 채택 gate로 고정했다. 합성 evidence만 검증했으며 실제 baseline 수치나 개선 성과는 없다.
