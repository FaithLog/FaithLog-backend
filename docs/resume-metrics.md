...
- 초기 #236 단독 gate 뒤 최종 통합에서는 poll 참여 shape를 제거하고 comment/penalty 및 V17 archive 계약을 반영해 다시 검증했다. 최종 exact-HEAD `./gradlew clean test build asciidoctor`는 `835 tests / failures 0 / errors 0 / skipped 14`와 REST Docs HTML 생성을 확인했다. 이 실행 시간은 제품 성능 수치가 아니다.
...
- Final whole-diff review found that the annual archive writer could pass a full 1,000-member year's source IDs through one `IN` predicate. A test-first correction bounds lookup and write batches to 500 while preserving idempotency and the archive/delete transaction. The final exact-HEAD clean gate is 835 tests / failures 0 / errors 0 / skipped 14 with build and Asciidoctor success; a disposable PostgreSQL 17 gate passed all 8 clean/upgrade migration cases without touching the shared QA database.
