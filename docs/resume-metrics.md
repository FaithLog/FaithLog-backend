...
- #198 알림 생성·전송 before 시나리오 준비 (`scenario-ready / not-measured`):
  ...
  - Firebase credential 존재, fake가 아닌 sender, `test|docker|prod` profile, 사용자 승인 project와 다른 Compose label, manifest의 Compose project/database 불일치, 실제 endpoint 불일치, dirty worktree, `faithlog-latest` shared stack을 즉시 거부한다. Spring context-startup coffee seed runner도 mock으로 대체한다. fixture는 `PERFORMANCE_198_DUMMY:` token과 fixture marker log만 허용하고 non-fixture token mutation, dedupe replay, upstream campus/user target equality, cross-user failure continuation과 mixed-token 사용자 내부 partial failure를 correctness 계약으로 검증한다.
  - 수집 준비 지표는 생성/전송/end-to-end duration·throughput p50/p95/p99/max, Hibernate prepared statements와 per-user DB calls, harness-lifecycle PostgreSQL/Redis counter delta, Java/container CPU·memory, log/token insert/update count, provider fake failure rate와 final log failure rate다. 다른 부하가 없는 전제에서 PostgreSQL notification log/token physical write와 Redis SET은 logical write의 exact count여야 하며 under/over-count를 모두 거부한다. sample은 현재 cold-JVM이고 warmup은 외부 PostgreSQL/Redis cache만 대상으로 한다. 누적 dummy history 왜곡을 막기 위해 pre-run token/log cardinality·relation bytes·Redis DBSIZE가 다른 sample은 집계를 거부하며, 실행 모델과 동일 snapshot 복원 또는 fixture cleanup 정책은 baseline 승인 때 확정한다. 실제 fixture/job/Docker/성능 DB/baseline은 실행하지 않았으므로 수치와 개선 성과는 `not measured`다.
  - TDD는 구현 파일 부재로 Node 계약 `5 tests / 5 failures` RED를 먼저 확인·커밋한 뒤 GREEN으로 전환했다. 리뷰 보강 후 최종 계약은 합성 PostgreSQL/Redis/Docker evidence의 verifier·summarizer 실행까지 포함한 `6 tests / 0 failures`다. Java harness compile, Bash/Node syntax, 전체 `450 tests / 0 failures / 4 skipped`, `./gradlew build`, `./gradlew asciidoctor`를 통과했으며 신규 guarded harness 1건은 기본 실행에서 정상 skip됐다.
