...
- #200 PM 최종 재리뷰 finding 3건 보강:
  ...
  - 프론트 계약 자체 리뷰: 일반 Poll 목록/상세가 전체 캠퍼스 투표를 반환하지만 COFFEE creator/관리 가능 필드가 없어 다른 담당자 투표의 write 버튼을 프론트가 안전하게 제어할 수 없는 누락을 발견했다. service/REST Docs RED에서 필드 부재를 compile failure로 재현하고 `createdByUserId`와 서버 계산 `manageableByMe`를 목록·상세에 추가했다. COFFEE는 ACTIVE duty + creator만 true, 비담당 관리자와 다른 담당자는 false이며 비-COFFEE는 기존 관리자 권한을 따른다.
