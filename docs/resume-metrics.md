...
- #194 PostgreSQL 실행계획 기반 index before-baseline 시나리오 준비:
  ...
  - 범위: #192/#193/#195/#196/#197/#198/#199의 핵심 read SQL 24개를 공통 1,000명 `datasetId + fixtureRunId`와 cross-issue report에 연결했다. 모든 SQL은 `SELECT`/read-only CTE이고 기존 row 수정·삭제, Flyway/index/HypoPG/extension 변경은 0건이다.
  - 측정 계약: future runner는 `EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON)`을 read-only transaction에서 순차 실행한다. 첫 관찰을 true cold cache로 부르지 않고 `cold_like_observation`, 후속을 `warm_cache`로 분리하며 둘 다 `cacheResetPerformed=false`다. before/after planner settings, `last_analyze`/`last_autoanalyze`/`n_mod_since_analyze`, external activity, 실제 Compose project/service/container identity를 함께 기록한다.
  - 비교 계약: 정규화 plan structure SHA-256과 Seq/Index/Index Only/Bitmap scan, estimated/actual rows, loops, shared hit/read, rows removed, sort, planning/execution time을 저장한다. runtime 숫자만 달라진 동일 구조는 같은 plan hash가 되며, candidate predicate/order/join column은 제안 후보일 뿐 성과나 DDL 승인으로 해석하지 않는다.
  - TDD/검증: production 변경 전 Node 계약 테스트가 구현 파일 부재로 `5 tests / 5 failures` RED였고, inventory/runner/normalizer/report contract 구현 후 `5 tests / 0 failures` GREEN이었다. Node/Bash syntax, JSON 24-query parse, SQL read-only safety, `git diff --check`가 성공했다. 첫 전체 Gradle은 macOS Byte Buddy dynamic attach 구간에서 장시간 정지해 해당 실행만 중단했고, 현재 resolved `byte-buddy-agent 1.17.8`을 test JVM에 선로딩해 `449 tests / 0 failures / 0 errors / 3 skipped`, build, asciidoctor를 성공했다. 실제 성능 수치와 이력서 개선 문장은 baseline 및 별도 index 승인 전까지 pending이다.
