...
- The exact-source clean gate completed in 7m14s with 147 suites / 747 tests / failures 0 / errors 0 / skipped 10, plus build and Asciidoctor success. The PostgreSQL V1→V14 and V13→V14 tests are implemented and environment-gated, but their actual run remained blocked because Docker Desktop returned daemon `EOF` and no local PostgreSQL port was listening; no shared QA DB or Docker container lifecycle was changed to work around that environment failure.
