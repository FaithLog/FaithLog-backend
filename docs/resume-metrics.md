...
- #198 알림 생성·전송 before 시나리오 준비 (`scenario-ready / not-measured`):
  ...
  - Firebase credential 존재, fake가 아닌 sender, `test|docker|prod` profile, 사용자 승인 project와 다른 Compose label, manifest의 Compose project/database 불일치, 실제 endpoint 불일치, `faithlog-latest` shared stack을 즉시 거부한다. fixture는 `PERFORMANCE_198_DUMMY:` token과 fixture marker log만 허용하고 non-fixture token mutation, dedupe replay, upstream campus/user target equality, permanent failure 뒤 후속 success continuation을 correctness 계약으로 검증한다.
  - 수집 준비 지표는 생성/전송/end-to-end duration·throughput p50/p95/p99/max, Hibernate prepared statements와 per-user DB calls, PostgreSQL/Redis counter delta, Java/container CPU·memory, log/token insert/update count, provider fake failure rate다. 실제 fixture/job/Docker/성능 DB/baseline은 실행하지 않았으므로 수치와 개선 성과는 `not measured`다.
