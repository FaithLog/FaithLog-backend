...
- 최종 retention 및 Asia/Seoul 자정 cron 변경 뒤 weeklymaterial/announcement/media/Flyway contract 회귀는 175 tests / failures 0 / errors 0 / skipped 0으로 통과했고 `build asciidoctor -x test -x jacocoTestReport`도 성공했다. 전체 `test build asciidoctor` 재시도는 관련 없는 `PasswordResetTransactionIntegrationTest` Spring context 생성 중 `OutOfMemoryError`가 발생해 중단했다.
