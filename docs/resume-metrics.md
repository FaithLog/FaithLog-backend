...
- #198 알림 생성·전송 before 시나리오 준비 (`scenario-ready / not-measured`):
  ...
  - 수집 준비 지표는 생성/전송/end-to-end duration·throughput p50/p95/p99/max, Hibernate prepared statements와 per-user DB calls, harness-lifecycle PostgreSQL/Redis counter delta, Java/container CPU·memory, log/token insert/update count, provider fake failure rate와 final log failure rate다. sample은 현재 cold-JVM이고 warmup은 외부 PostgreSQL/Redis cache만 대상으로 한다. 누적 dummy history 왜곡을 막기 위해 pre-run token/log cardinality·relation bytes·Redis DBSIZE가 다른 sample은 집계를 거부하며, 실행 모델과 동일 snapshot 복원 또는 fixture cleanup 정책은 baseline 승인 때 확정한다. 실제 fixture/job/Docker/성능 DB/baseline은 실행하지 않았으므로 수치와 개선 성과는 `not measured`다.
