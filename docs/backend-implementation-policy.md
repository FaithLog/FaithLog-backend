...
- A valid Monday week lookup is a normal empty-state page even when neither slot exists and returns both nullable fields as `null`; list pagination still contains only weeks with ACTIVE material rows.
- The private media access policy includes only same-campus ACTIVE weekly attachments for an ACTIVE member. The service-ADMIN manager bypass does not grant public member reads.
- Media attachment exclusivity is symmetric: after the stable media row lock, weekly, announcement, and poll attachment flows all query the other consumer-owned ports before relation mutation.
...
- Manual and retention deletion lock material then outbox and atomically mark a still-pending first-publication row processed, while retaining the row as dedupe history. Processor delivery uses an immutable scalar snapshot followed by material lock and outbox lock, so concurrent processors send exactly once and processor/delete races serialize as send-before-delete or suppress-before-send.
- A physically deleted historical slot may be registered again, but a locked existing outbox history prevents creation of another outbox and notification. Duplicate-key exceptions are not normal control flow.
- Admin PUT command and manager-authorized response assembly share one transaction; a global service ADMIN without membership receives the management response, while public GET still requires ACTIVE membership and response assembly failure rolls back the PUT.
- A weekly-material slot reaches retention at Asia/Seoul midnight on `weekStartDate.plusMonths(3)`. The scheduler row-locks each due slot, marks an attached READY PDF ORPHANED, and physically deletes the `weekly_materials` row in one transaction. DELETED tombstones are also physically removed at that boundary. The durable outbox and notification logs remain as history, and the existing cleanup retry/lease owns later R2 object deletion.
