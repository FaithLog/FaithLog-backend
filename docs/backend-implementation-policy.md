...
- User and administrator charge-status writes must acquire the same charge-row write lock before validating and applying a transition. Concurrent attempts against one charge must serialize so the later request observes the committed terminal state and returns the existing conflict response instead of overwriting it.
