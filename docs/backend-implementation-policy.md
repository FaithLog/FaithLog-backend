...
- Independently of the manager deletion API, announcement retention is due at Asia/Seoul midnight on the calendar date `publishedAt.toLocalDate(Asia/Seoul).plusMonths(3)`. The scheduler covers both PUBLISHED and ARCHIVED announcements, excludes unpublished SCHEDULED announcements, locks and rechecks each row, ORPHANs linked image/PDF assets, deletes attachment links and the announcement in one transaction, and preserves announcement outbox and notification-log history. Existing cleanup retry/lease owns the later R2 deletion.
