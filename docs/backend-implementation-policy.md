...
- User and administrator charge-status writes, and existing-source charge update/reactivation, must acquire the same charge-row write lock before validating and applying a transition. Concurrent attempts against one charge must serialize so the later request observes the committed state and returns the existing conflict response instead of overwriting it. This source-key locking applies to both `PENALTY` and `COFFEE` upserts.
