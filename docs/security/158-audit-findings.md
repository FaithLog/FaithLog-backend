...
| 후보 | 판정 | 근거 |
| --- | --- | --- |
| logout 후 같은 session의 과거 access token 잔존 | **의도된 현재 정책/PM 재확인 필요** | 현재 문서와 테스트는 제시된 access JTI 1개 + current refresh key 삭제를 요구한다. refresh 전 access와 후 access가 겹치면 과거 access는 자체 만료까지 남을 수 있다. ASVS session 종료 의미와 사용자-facing logout 범위는 U-158-03에서 확인 필요 |
