...
class YearlyRecapControllerTest {
	...
	void no_data_returns_an_exact_zero_payload_without_private_fields() throws Exception {
		...
		mockMvc.perform(get("/api/v1/users/me/yearly-recaps/previous")
			...
			.andExpect(jsonPath("$.data.pollActivity").doesNotExist())
			.andExpect(jsonPath("$.data.commentActivity.writtenCount").value(0))
			.andExpect(jsonPath("$.data.penaltySummary.totalCount").value(0))
			.andExpect(jsonPath("$.data.penaltySummary.totalAmount").value(0))
			.andExpect(jsonPath("$.data.penaltySummary.paidCount").value(0))
			.andExpect(jsonPath("$.data.penaltySummary.paidAmount").value(0))
			.andExpect(jsonPath("$.data.penaltySummary.unpaidCount").value(0))
			.andExpect(jsonPath("$.data.penaltySummary.unpaidAmount").value(0))
