...
2. PM 세션의 명시적 머지 승인 전에는 개발 세션이 스스로 push, PR 생성, merge를 수행하지 않는다.
...
6. PM finding이 0건이고 필수 검증이 모두 통과한 경우에만 PM 세션이 develop 대상 PR을 생성하고 CI를 확인해 머지한다.
7. CI 실패나 PR 충돌이 있으면 이슈를 완료 처리하지 않고 원인을 수정, 재검증, 재리뷰한다.
8. develop 머지와 Issue 종료 또는 완료 상태가 실제 확인되어야 이슈가 최종 완료된다.
