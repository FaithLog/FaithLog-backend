...
test('cross-issue artifacts are real, unique, in-tree accepted JSON with matching identity and hashes', async () => {
	...
	try {
		...
		const accepted = {
			...
			196: { status: 'scenario-ready', accepted: true, measurementStatus: 'accepted' },
...
test('anchors require exact accepted-artifact identity and read-only relationship/cardinality preflight', async () => {
	...
	assert.equal(validateAnchorPreflight(valid).adoptable, true);
	const mismatch = validateAnchorPreflight({ ...valid, memberInCampus: false, pollCount: 0 });
...
test('source identity binds SQL bytes, inventory, report contract, production refs, git commit and clean state', async () => {
	...
	try {
		...
		const identity = buildSourceIdentity({
			...
			inventoryPath: 'inventory.json', reportContractPath: 'report-contract.json', expected,
		});
...
test('schema identity canonicalizes Flyway, columns, constraints and indexes and rejects changes', async () => {
	const { normalizeSchemaState, validateSchemaContinuity } = await import(moduleUrl('schema-contract.mjs'));
	const state = {
...
test('query-window monitor retains transient external activity that disappears before the final snapshot', async () => {
	const { observeActivitySamples } = await import(moduleUrl('activity-monitor-contract.mjs'));
	const result = await observeActivitySamples(async (sampleNumber) => [
		[],
		[{ pid: 902, applicationName: 'frontend', state: 'active', queryStart: '2026-07-14T00:00:01Z' }],
		[],
	][sampleNumber], { sampleCount: 3, observerPid: 901, measuredApplicationName: 'faithlog-194-run-1' });
	assert.equal(result.adoptable, false);
	assert.equal(result.transientExternalActivityDetected, true);
	assert.equal(result.sessions[0].pid, 902);
});
