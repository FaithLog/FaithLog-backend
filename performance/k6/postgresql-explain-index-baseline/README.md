...
- #192/#193/#195/#196/#197/#198/#199별 accepted JSON artifact의 상대 경로
- inventory SQL에 주입할 `anchors`

구조 예시는 [`cross-issue-report.example.json`](cross-issue-report.example.json)에 있다. 예시 ID와 경로는 측정값이 아니며 실제 실행에 그대로 사용하지 않는다. 각 artifact는 `issueNumber`, `datasetId`, `fixtureRunId`, `memberCount=1000`, `expectedAnchors`, 해당 이슈가 이미 정의한 성공 상태를 포함해야 한다. 허용 상태는 #192 `verified+passed`, #193 `eligible-for-pm-review`, #195 `adoptable`, #196 `accepted+measurementStatus=measured`, #197 `baseline-measured`, #198 `before-baseline`, #199 `adoptable`이며 `scenario-ready/not-measured`는 허용하지 않는다. runner는 cross report가 있는 디렉터리를 기준으로 경로를 resolve하고 절대 경로, `..`, symlink, 디렉터리, 중복 real path를 거부한다. 모든 artifact의 JSON schema/identity/status와 SHA-256을 검증하며 하나라도 미측정·미채택이면 Docker/psql/EXPLAIN 전에 중단한다.
...
runner는 inspect 결과의 `POSTGRES_DB`, container network address, PostgreSQL 내부 port, container start time과 psql이 반환한 `inet_server_addr()`, `inet_server_port()`, `current_database()`, `pg_postmaster_start_time()`을 비교한다. 하나라도 일치하지 않으면 EXPLAIN 전에 중단한다. 측정 후에도 identity를 다시 수집해 endpoint/database/postmaster가 바뀌면 report를 invalid 처리한다. 따라서 독립 `PGHOST`/`PGPORT`가 다른 로컬·원격 DB 또는 같은 container의 다른 database를 가리킨 상태로 inspected container identity를 보고할 수 없다.
...
runner는 수동 `ANALYZE`를 실행하지 않는다. `EXPLAIN ANALYZE` 수집 시각은 각 query run의 `capturedAt`으로 별도 기록한다. 시작 snapshot의 필수 setting/table/schema evidence가 없거나 runner 외 active session이 있으면 어떤 EXPLAIN도 실행하지 않는다. 전체 query/phase 측정 구간에는 하나의 연속 read-only observer가 50ms 간격과 window 종료 시점에 `pg_stat_activity`를 sampling한다. runner는 psql을 만들기 전에 query별 CSPRNG application name, registration token, label을 observer에 `PREPARE`해 같은 prefix의 다른 session도 첫 관측부터 외부 activity로 구분한다. measured psql이 backend PID와 backend start instant를 공개하면 prepared token에 exact identity를 `BIND`하고, observer가 idle 대기 상태를 포함해 이를 실제 관측한 token ACK 뒤에만 EXPLAIN을 시작한다. psql 종료 뒤에는 token unregister ACK까지 받아 PID 재사용이 과거 측정 identity로 제외되지 않게 하며, 최종 label/count/lifecycle도 예정 workload와 정확히 비교한다. 미등록 active session이 관측되거나 observer 자체가 종료되면 main runner가 진행 중 psql을 즉시 종료해 partial invalid report로 중단한다. observer 자신의 PID와 등록·관측 중인 exact identity만 제외하므로 같은 prefix/application name을 사용한 외부 session 및 중간에 시작했다가 끝난 외부 session도 누적 기록한다. 실행 중 planner settings, postmaster, analyze/autoanalyze, vacuum/autovacuum, visibility, schema/index fingerprint, DB identity가 바뀌거나 외부 activity가 관측되면 invalid/pending report를 남기고 non-zero 종료한다.
