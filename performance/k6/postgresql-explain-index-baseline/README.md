...
runner는 수동 `ANALYZE`를 실행하지 않는다. `EXPLAIN ANALYZE` 수집 시각은 각 query run의 `capturedAt`으로 별도 기록한다. 시작 snapshot의 필수 setting/table statistics가 없거나 runner 외 active session이 있으면 어떤 EXPLAIN도 실행하지 않는다. 같은 application name의 다른 session도 외부 activity로 센다. 실행 중 planner settings, postmaster start, `last_analyze`, `last_autoanalyze`, `n_mod_since_analyze`, DB identity가 바뀌거나 종료 snapshot에 외부 active session이 있으면 report status를 `invalid-pending-planner-integrity`로 기록하고 non-zero 종료한다. 이 report는 baseline으로 채택할 수 없다.
