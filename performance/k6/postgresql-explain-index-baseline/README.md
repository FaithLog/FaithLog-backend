...
runner는 pre-lock inspect 뒤 canonical lock을 획득하고 즉시 같은 container를 다시 inspect한다. container ID/name, image ID/reference, start time, Compose project/service/config/working directory, `POSTGRES_DB`, internal port와 정규화 network identity가 하나라도 바뀌면 psql/schema/anchor/EXPLAIN을 0회로 중단한다. 측정 종료에도 세 번째 inspect로 같은 immutable identity continuity를 검증한다. lock 직후 inspect 결과의 database/network/start time과 psql이 반환한 `inet_server_addr()`, `inet_server_port()`, `current_database()`, `pg_postmaster_start_time()`도 실행 전후 비교한다. 따라서 lock 사이 container 교체, 독립 `PGHOST`/`PGPORT`가 다른 로컬·원격 DB를 가리키는 상태, 같은 container의 다른 database 접속을 승인 target 결과로 보고할 수 없다.
