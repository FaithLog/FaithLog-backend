...
function psql(sql) { const result = spawnSync('docker', ['exec', '-i', POSTGRES_CONTAINER_ID, 'psql', '-U', target.database.user, '-d', target.database.name, '-X', '-q', '-v', 'ON_ERROR_STOP=1', '-A', '-t'], { input: sql, encoding: 'utf8' }); if (result.status !== 0) throw new Error(`psql failed: ${result.stderr.trim()}`); return result.stdout.trim(); }
