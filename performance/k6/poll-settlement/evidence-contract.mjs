...
export function validateMetricEvidence(evidence, mode, expectedCase) {
	...
	const requests = metric(evidence.metrics, `${kind}_settlement_requests`);
	const count = finite(requests.count, 'metric count');
	if (!Number.isSafeInteger(count) || count !== expectedCount) throw new Error(`metric count expected ${expectedCount}`);
	if (rate(metric(evidence.metrics, `${kind}_settlement_failure_rate`), 'failure') !== 0) throw new Error('failure must be exact zero');
	if (rate(metric(evidence.metrics, 'http_req_failed'), 'HTTP failure') !== 0) throw new Error('HTTP failure must be exact zero');
	if (rate(metric(evidence.metrics, 'checks'), 'checks') !== 1) throw new Error('checks must be exact one');
	const iterations = finite(metric(evidence.metrics, 'iterations').count, 'iteration count');
	if (!Number.isSafeInteger(iterations) || iterations !== expectedCount) throw new Error(`iteration count expected ${expectedCount}`);
	const duration = metric(evidence.metrics, `${kind}_settlement_duration`);
	...
	const startMs = finite(metric(evidence.metrics, `${kind}_settlement_started_at`).min, 'metric start');
	const endMs = finite(metric(evidence.metrics, `${kind}_settlement_finished_at`).max, 'metric end');
...
export function validateWarmupEvidence(evidence, mode, expectedCase) {
	...
	const count = finite(metric(evidence.metrics, `${kind}_warmup_requests`).count, 'warmup count');
	if (!Number.isSafeInteger(count) || count !== 1) throw new Error('warmup count expected 1');
	if (rate(metric(evidence.metrics, `${kind}_warmup_failure_rate`), 'warmup failure') !== 0) throw new Error('warmup failure must be exact zero');
	if (rate(metric(evidence.metrics, 'http_req_failed'), 'warmup HTTP failure') !== 0) throw new Error('warmup HTTP failure must be exact zero');
	if (rate(metric(evidence.metrics, 'checks'), 'warmup checks') !== 1) throw new Error('warmup checks must be exact one');
	const iterations = finite(metric(evidence.metrics, 'iterations').count, 'warmup iteration count');
...
function validateRuntime(snapshot, target, expectedCase) {
	...
	for (const role of RESOURCE_ROLES) {
		validateContainerTarget(target.containers[role], role);
		if (stable(snapshot.containers[role]) !== stable(target.containers[role])) throw new Error(`runtime target drift: ${role}`);
	}
...
function metric(metrics, name) { const raw = metrics[name]; const value = raw?.values ?? raw; if (!value || typeof value !== 'object') throw new Error(`metric missing: ${name}`); return value; }
function rate(value, label) { const keys = ['rate', 'value'].filter((key) => Object.hasOwn(value, key)); if (keys.length !== 1) throw new Error(`${label} metric shape`); return finite(value[keys[0]], label); }
