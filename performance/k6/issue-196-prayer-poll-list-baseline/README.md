...
Boundary validation now selects the latest sample at or before measured start and the earliest sample at or after measured end, requires each selected boundary to be within the unchanged approved max gap, and computes the minimum sample count over that selected coverage segment. Every sample still must have exact container identity, strict monotonic timestamps, and consecutive gap within the approved maximum; later valid post-processing samples are allowed but cannot replace the nearest post-end boundary. Synthetic RED/GREEN and read-only revalidation of the preserved `h02` endpoint both leave only `adoption-policy-pending-user-approval`. `h02` remains immutable, rejected, diagnostic-only, and non-reusable. The `h03` prep identifier was then consumed by a stale tooling-HEAD preflight before any lifecycle, fixture, or load operation and is also non-reusable.

Fresh `h04` passed app-only runtime prep, fixture preparation, shaping, and the first two measured Prayer endpoints with HTTP/custom failure `0`. Diagnostic-only `prayer_current_season` recorded 88,169 requests, p50 5.41 ms, p95 12.14 ms, p99 24.30 ms, max 308.47 ms, and 734.635 requests/s; `prayer_groups` recorded 2,041 requests, p50 256.50 ms, p95 526.80 ms, p99 778.71 ms, max 1.48 s, and 16.953 requests/s. These partial values are not a baseline or performance claim.

The third endpoint, `prayer_assignable`, stopped after a 245-request warmup because all HTTP responses passed but the k6 correctness predicate rejected the shared campus creator. Seed intentionally uses the same runtime ADMIN as creator of both campuses, so primary and isolation membership sets overlap by exactly that one user; the remaining 49 users are isolation-only. A one-request sanitized API diagnostic proved status 200, exact 1,000 primary IDs in order, all assigned, no unexpected primary user, and a separate installed-k6 diagnostic isolated only the old all-isolation-members predicate as false. Temporary diagnostic ADMIN accounts were restored to USER.

The scenario now derives the isolation deny-set as `isolation user IDs - primary user IDs` and uses that exact contract in both assignable and weekly-board validators. Malformed, duplicate, or non-positive member IDs still fail closed. Test-only RED reproduced the missing helper, focused GREEN and installed k6 v2 inspect passed, and the full issue-local suite is `53/53` GREEN. `h04` is immutable, rejected, diagnostic-only, and non-reusable; the next actual must use fresh `h05` runtime-prep, fixture, and execution identifiers.
