...
PostgreSQL/Redis counter와 Docker stats의 window는 Gradle 실행, Spring startup, measured method, correctness replay/postflight를 모두 포함하는 `gradle-spring-harness-lifecycle`이다. creation/delivery phase duration 또는 per-user Hibernate prepared statements와 직접 대응시키지 않으며 harness-wide 보조 evidence로만 집계한다. verifier는 request marker log 총수 증가, active dummy-token 감소, retained Redis key 증가를 exact count로 맞추고 PostgreSQL stats counter가 logical write 하한을 충족하는지 확인한다. Redis SET은 creation+replay 2,000회와 delivery lock 1회의 exact count를 요구하며 under/over-count 합성 계약 테스트를 함께 둔다.
