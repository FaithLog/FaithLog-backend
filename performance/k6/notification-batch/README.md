...
fixture preparation과 runner 모두 `/tmp/faithlog-performance-<actualComposeProject>.lock`을 사용하므로 다른 worktree의 performance baseline, frontend QA, Docker QA, 부하와도 병렬 실행하지 않는다. runner는 실제 Compose label/host port/image ID/git commit, PostgreSQL counter delta, Redis `INFO commandstats` delta, PostgreSQL/Redis `docker stats` peak, Java process CPU duration/heap delta를 기록하고 verifier가 evidence 파일을 파싱한다. run ID가 이미 있으면 overwrite하지 않고 즉시 거부한다.
...
현재 campus isolation은 scheduler가 같은 campus ACTIVE member ID만 command에 전달한다는 production 계약에 의존한다. request service 자체는 전달된 target ID의 campus membership을 다시 검증하지 않는다. harness는 datasetId와 campus name을 DB에서 다시 묶고 same-campus ACTIVE member 1,000명 목록과 request log 사용자 집합의 exact equality를 검증한다. report에는 `requestServiceRevalidatesCampusMembership=false`를 명시해 이 경계를 격리 보장으로 과장하지 않는다. production 권한/트랜잭션 의미는 바꾸지 않는다.
