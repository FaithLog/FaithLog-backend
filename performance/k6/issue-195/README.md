...
- Execution D and its report directory are non-reusable and remain read-only. The preserved B/C/D reports are not modified, cleaned, copied, or printed by this correction.
- The preserved dataset/fixture are handed to fresh `EXEC195_BEFORE_20260716_E`; provisioning and fixture must not be repeated or cleaned up. After D stopped, the PM verified the broad `perf-195` temporary service-role state as 0 `ADMIN` and 4 `USER`, plus lock/process cleanup. This scenario-only correction does not query or mutate those accounts.
...
- `PERF_EXECUTION_RUN_ID=EXEC195_BEFORE_20260716_E` (`..._A`, `..._B`, `..._C`, and `..._D` are rejected and non-reusable)
...
4. Run `run-baseline.sh` once with fresh `PERF_EXECUTION_RUN_ID=EXEC195_BEFORE_20260716_E` and all remaining required API/DB/runtime inputs. Never reuse executions A/B/C/D or repeat the successful additive preparation.
...
The 22 k6 processes have exactly 27 minutes 30 seconds of configured load time. Provisioning and fixture time are already complete and are not repeated; the remaining planning allowance, not a measured result, is 10–25 minutes for per-case login/evidence/validation around k6 in addition to the configured load time. Stop at the first lock, source/target/runtime identity, credential, manifest, report collision, token-lifetime, summary, correctness, DB/query/table, resource cadence, or first-rejection gate failure. Preserve all evidence and additive rows; do not cleanup or continue after failure.
