...
- Execution B and its report directory are non-reusable and remain read-only. The preserved dataset/fixture are handed to fresh `EXEC195_BEFORE_20260716_C`; provisioning and fixture must not be repeated or cleaned up.
- After B stopped, the PM verified the temporary service-role state as 0 `ADMIN` and 1 `USER`. This scenario-only correction does not query or mutate that account.
...
- `PERF_EXECUTION_RUN_ID=EXEC195_BEFORE_20260716_C` (`..._A` and `..._B` are rejected and non-reusable)
...
3. Preserve the PM-confirmed 0 `ADMIN` / 1 `USER` service-role state. Pass the preserved fixture manifest's primary campus, isolation campus, and isolation user IDs exactly as `CAMPUS_ID`, `ISOLATION_CAMPUS_ID`, and `ISOLATION_USER_ID`; keep the dataset member password out of the runner environment.
4. Run `run-baseline.sh` once with fresh `PERF_EXECUTION_RUN_ID=EXEC195_BEFORE_20260716_C` and all remaining required API/DB/runtime inputs. Never reuse executions A/B or repeat the successful additive preparation.
