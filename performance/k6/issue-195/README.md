...
This directory prepares the before-only local Docker baseline for Issue #195. This scenario-correction session did not run a load, and this directory does not contain an adopted baseline or an improvement claim. The PM-controlled actual checkpoint below records successful provisioning/fixture preparation plus rejected executions A through F; none produced an adoptable baseline.
...
- The preserved dataset/fixture are handed to fresh `EXEC195_BEFORE_20260716_G`; provisioning and fixture must not be repeated or cleaned up. This scenario-only correction does not query or mutate the temporary accounts.
- Before F, PM independently ran a 2-second read-only control smoke after the E tooling correction. It produced `status=supporting-only`, `automaticAdoption=false`, `configuredDuration=2s`, raw control commit delta 2, background/unattributed delta 1, rollback delta 0, and `backgroundSubtractionApplied=false`. This is smoke diagnostic evidence only, not a baseline or performance achievement. The separate Issue #208 common audit gate was completed before F; the next handoff is fresh G.
...
- `PERF_EXECUTION_RUN_ID=EXEC195_BEFORE_20260716_G` (`..._A` through `..._F` are rejected and non-reusable)
...
4. Run `run-baseline.sh` once with fresh `PERF_EXECUTION_RUN_ID=EXEC195_BEFORE_20260716_G` and all remaining required API/DB/runtime inputs. Never reuse executions A through F or repeat the successful additive preparation.
