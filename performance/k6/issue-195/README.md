...
| `duty_assignments` | `GET /api/v1/admin/campuses/{campusId}/duty-assignments` | none | active service `ADMIN`, or ACTIVE campus `MINISTER`/`ELDER`/`CAMPUS_LEADER` | `campus_duty_assignments.id asc`; active only |

The member and duty endpoints are intentionally not given invented pagination or filter parameters. Role/search/page combinations apply to the service-admin user API; status/search/page combinations apply to the service-admin campus API.
...
- `duty_assignments`: one unpaginated full-list run against 101 active assignments.
