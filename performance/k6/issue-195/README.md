...
- The preserved dataset/fixture are handed to fresh `EXEC195_BEFORE_20260716_D`; provisioning and fixture must not be repeated or cleaned up. After C stopped, the PM verified the broad `perf-195` temporary service-role state as 0 `ADMIN` and 3 `USER`. This scenario-only correction does not query or mutate those accounts.
...
- `PERF_EXECUTION_RUN_ID=EXEC195_BEFORE_20260716_D` (`..._A`, `..._B`, and `..._C` are rejected and non-reusable)
...
3. Preserve the PM-confirmed broad `perf-195` temporary service-role state of 0 `ADMIN` / 3 `USER`. Pass the preserved fixture manifest's primary campus, isolation campus, and isolation user IDs exactly as `CAMPUS_ID`, `ISOLATION_CAMPUS_ID`, and `ISOLATION_USER_ID`; keep the dataset member password out of the runner environment.
4. Run `run-baseline.sh` once with fresh `PERF_EXECUTION_RUN_ID=EXEC195_BEFORE_20260716_D` and all remaining required API/DB/runtime inputs. Never reuse executions A/B/C or repeat the successful additive preparation.
