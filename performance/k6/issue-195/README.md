...
- Execution A and its report directory are non-reusable. The preserved dataset/fixture are handed to the fresh `EXEC195_BEFORE_20260716_B`; provisioning and fixture must not be repeated or cleaned up.
- The temporary ADMIN elevation used by the PM flow must be confirmed restored to `USER` by the PM trap before execution B. This scenario-only correction does not query or mutate that account.
...
- `PERF_EXECUTION_RUN_ID=EXEC195_BEFORE_20260716_B` (`..._A` is rejected and non-reusable)
...
1. Do not rerun provisioning or fixture creation. Read the preserved success manifests for `PERF_1000_20260716_195_A` and `ISSUE195_BEFORE_20260716_A`, confirm their exact IDs/cardinalities, and confirm `EXEC195_BEFORE_20260716_B` has no report path. Execution A and its report remain preserved but rejected.
...
3. Confirm the PM trap restored the temporary ADMIN to `USER`. Pass the preserved fixture manifest's primary campus, isolation campus, and isolation user IDs exactly as `CAMPUS_ID`, `ISOLATION_CAMPUS_ID`, and `ISOLATION_USER_ID`; keep the dataset member password out of the runner environment.
4. Run `run-baseline.sh` once with fresh `PERF_EXECUTION_RUN_ID=EXEC195_BEFORE_20260716_B` and all remaining required API/DB/runtime inputs. Never reuse execution A or repeat the successful additive preparation.
