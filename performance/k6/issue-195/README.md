...
This directory prepares the before-only local Docker baseline for Issue #195. Scenario authoring did not run a load, and this directory does not contain a measured baseline or an improvement claim. The PM-controlled actual checkpoint below records successful provisioning/fixture preparation plus rejected executions A through E; none produced an adoptable baseline.
...
- The preserved dataset/fixture are handed to fresh `EXEC195_BEFORE_20260716_F`; provisioning and fixture must not be repeated or cleaned up. This scenario-only correction does not query or mutate the temporary accounts.
- PM independently ran a 2-second read-only control smoke after the tooling correction. It produced `status=supporting-only`, `automaticAdoption=false`, `configuredDuration=2s`, raw control commit delta 2, background/unattributed delta 1, rollback delta 0, and `backgroundSubtractionApplied=false`. This is smoke diagnostic evidence only, not a baseline or performance achievement. Fresh F must not run until the separate Issue #208 common audit is complete.
...
- `PERF_EXECUTION_RUN_ID=EXEC195_BEFORE_20260716_F` (`..._A` through `..._E` are rejected and non-reusable)
...
1. Do not rerun provisioning or fixture creation. Read the preserved success manifests for `PERF_1000_20260716_195_A` and `ISSUE195_BEFORE_20260716_A`, confirm their exact IDs/cardinalities, and confirm `EXEC195_BEFORE_20260716_F` has no report path. Executions A through E and their reports remain preserved but rejected.
2. Re-read source checkout cleanliness, full app/PostgreSQL/Redis identities, actual service labels, published endpoint, PostgreSQL server/postmaster identity, and the preserved manifest target IDs. Stop before login if any value differs.
3. Preserve the PM-confirmed broad `perf-195` temporary service-role state of 0 `ADMIN` / 4 `USER`. Pass the preserved fixture manifest's primary campus, isolation campus, and isolation user IDs exactly as `CAMPUS_ID`, `ISOLATION_CAMPUS_ID`, and `ISOLATION_USER_ID`; keep the dataset member password out of the runner environment.
4. After the separate Issue #208 common audit completes, run `run-baseline.sh` once with fresh `PERF_EXECUTION_RUN_ID=EXEC195_BEFORE_20260716_F` and all remaining required API/DB/runtime inputs. Never reuse executions A through E or repeat the successful additive preparation.
5. The current recommended handoff values are `WARMUP_VUS=1`, `WARMUP_DURATION=30s`, `MEASURED_VUS=10`, `MEASURED_DURATION=2m`, `MAX_FAILURE_RATE=0`, `TOKEN_SAFETY_MARGIN_SECONDS=120`, and `RESOURCE_BOUNDARY_MAX_GAP_SECONDS=10`. They are recommendations, not hard-coded defaults or measured/approved results; PM supplies them explicitly at runtime under the user's automatic sequential-load decision. `K6_BIN`, service labels, API/DB credentials, and all reverified immutable identities remain required inputs.

The 22 k6 processes have exactly 27 minutes 30 seconds of configured load time. Eleven immediately-before idle controls add exactly 22 configured minutes at the recommended 2-minute measured duration, for 49 minutes 30 seconds of serialized warmup/control/measured time before login/evidence/validation overhead. Provisioning and fixture time are already complete and are not repeated; this is a planning duration, not a measured result. Stop at the first lock, source/target/runtime identity, credential, manifest, report collision, token-lifetime, control, summary, correctness, DB/query/table, resource cadence, or first-rejection gate failure. Preserve all evidence and additive rows; do not cleanup or continue after failure.
