...
Status: **fresh O before baseline manually adopted by PM; production optimization RED pending**.

이 디렉터리는 Issue #193의 current-develop 호환 correctness와 before 측정 계약을 보존한다. B부터 N까지의 actual attempt와 M conditional evidence는 아래 사유로 거부됐고, fresh O만 PM의 host-exclusive 검토를 거쳐 유효 before baseline으로 수동 채택됐다.

이전 상태 `scenario and runner/evidence contract-ready, fake/static verified, not measured`는 O 채택 전 단계의 이력이며 현재 상태가 아니다.
...
Fresh O는 모든 technical validator를 통과했고 PM이 host-exclusive window, 계정 복구, lock/process, credential 비노출을 독립 검증해 유효 before baseline으로 수동 채택했다. Runner 자체 classification은 계속 `conditional-shared-stack`, `automaticAdoption=false`이며 PM 수동 채택 사실과 분리해 보존한다. 이 목표의 k6/fixture/validator/test/docs 변경은 별도 사용자 승인 없이 TDD와 PM 리뷰로 진행하지만, `src/main` production backend 또는 Flyway 변경 직전에는 사용자 승인을 받아야 한다. 개발 세션은 actual Docker/DB/k6 after를 실행하지 않는다.

실제 수집은 개발 세션에서 실행하지 않는다.

채택된 O 식별자는 `I193_BEFORE_20260716_O / I193_FIXTURE_20260716_O / EXEC193_BEFORE_20260716_O`이고 report 경로는 `build/reports/k6/issue-193/I193_BEFORE_20260716_O/I193_FIXTURE_20260716_O/EXEC193_BEFORE_20260716_O`이다. B~O namespace와 report는 모두 immutable evidence로 보존하며 재사용하지 않는다.
...
## Adopted actual-before baseline O (2026-07-16)

`I193_BEFORE_20260716_O / I193_FIXTURE_20260716_O / EXEC193_BEFORE_20260716_O`는 source `origin/develop 6796ed146244d8f3f5b5dd7048ebe16865084a97`의 유효 before baseline으로 PM이 수동 채택했다. Fixture는 ACTIVE member 1,000명, account 5개, charge 35,000개 exact shape이며 correctness, archive cutoff, #200 duty ownership, #206 ordering gate를 모두 통과했다.

Measured window는 `2026-07-16T07:27:07.498Z`부터 `07:30:10.197Z`까지다. 16 cases 각각 484건, 총 7,744 HTTP와 check 23,232건을 failure 0으로 완료했다. 전체 throughput은 `42.483669 req/s`, case별 p50은 `109.60~251.62ms`, p95는 `208.54~423.45ms`, worst max는 `1293.83ms`다.
